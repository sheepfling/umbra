...
class UmbraJniRtiFactoryTest(unittest.TestCase):
    def test_package_declares_the_standard_factory_entry_point(self) -> None:
        ...
        self.assertRegex(
            metadata,
            r'(?m)^umbra-jni\s*=\s*'
            r'"umbra\.\_java\.rti1516_2025:UmbraJniRtiFactory"\s*$',
        )
    ...
    def test_factory_is_discoverable_from_the_standard_api_namespace(self) -> None:
        ...
        if not any(
            point.name == UmbraJniRtiFactory.ENTRY_POINT_ALIAS
            for point in entry_points(group="hla.rti1516_2025.factories")
        ):
            self.skipTest(
                "JNI entry-point metadata is available only after installing umbra-rti-jpype"
            )
        ...
        discovered = RtiFactoryFactory.getRtiFactory(UmbraJniRtiFactory.ENTRY_POINT_ALIAS)
        self.assertIsInstance(discovered, UmbraJniRtiFactory)
    ...
    def test_configuration_only_adapter_delegates_to_java_factory(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            factory = UmbraJniRtiFactory(api, bridge, native, runtime=runtime)
            ...
            self.assertEqual(
                ...
                UmbraJniRtiFactory.JAVA_FACTORY_NAME,
            )
            self.assertTrue(
                any(
    ...
    def test_artifact_directory_is_a_lazy_configuration_source(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            old = {
                ...
                for name in (
                    UmbraJniRtiFactory.ARTIFACT_DIRECTORY_ENVIRONMENT_VARIABLE,
                    UmbraJniRtiFactory.API_JAR_ENVIRONMENT_VARIABLE,
                )
            }
            try:
                os.environ[UmbraJniRtiFactory.ARTIFACT_DIRECTORY_ENVIRONMENT_VARIABLE] = str(root)
                os.environ[UmbraJniRtiFactory.API_JAR_ENVIRONMENT_VARIABLE] = str(api)
                runtime = _FakeRuntime()
                factory = UmbraJniRtiFactory(runtime=runtime)
    ...
    def test_artifact_directory_discovers_one_adjacent_api_jar(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            runtime = _FakeRuntime()
            factory = UmbraJniRtiFactory(artifact_directory=root, runtime=runtime)
