...
def _require_opaque_data(value: BytesLike) -> bytes:
    return copy_bytes(value, name="HLAopaqueData value")
...
class DataElement(ABC):
    """Abstract provider-owned encoding element.

    Java surface: ``hla.rti1516_2025.encoding.DataElement``.
    C++ surface: ``RTI::VariableLengthData``-backed concrete data elements.
    """
    ...
    @abstractmethod
    def decode(self, bytes_: BytesLike) -> Self:
...
class EncoderFactory(ABC):
    ...
    Java surface: ``hla.rti1516_2025.encoding.EncoderFactory``.
    C++ surface: concrete ``RTI::encoding`` basic-data-element constructors.
    """
