"""Run a small JPype handoff against the provider that just passed the Java TCK."""
...
def main() -> int:
    ...
    try:
        ...
        if args.native_library:
            jvm_args.append(f"-Dumbra.rti.jni.library={args.native_library.resolve()}")
        classpath = [str(args.api_jar.resolve()), *(str(item.resolve()) for item in args.provider_jar)]
