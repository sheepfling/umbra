...
foreach ($configuration in @($configurations)) {
    ...
    $dependencyJars = @()
    $dependencyProperty = $configuration.PSObject.Properties['dependencyJars']
    if ($null -ne $dependencyProperty -and $null -ne $dependencyProperty.Value) {
        $dependencyJars = @($dependencyProperty.Value)
    }
    ...
    $jvmArgumentProperty = $configuration.PSObject.Properties['jvmArguments']
    if ($null -ne $jvmArgumentProperty -and $null -ne $jvmArgumentProperty.Value) {
        $arguments.JvmArgument = @($jvmArgumentProperty.Value)
    }
    & $runScript @arguments
