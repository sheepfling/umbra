...
`umbra_rti_test_support.ConnectionFoundationConformanceMixin` and
...
The shared `iter_callback_provenance_matrix()` adds a five-case parity vector
for discovery, untimed value delivery, and timestamped value delivery.  The
2010 proxy test verifies producer and region data through supplemental-info
records; the 2025 proxy test verifies the direct producer/region positions.
The 2010 JNI type-roundtrip test and the 2025 JNI carrier test additionally
feed those vectors with JNI-produced Java handles, maps, times, and
retractions. Both assert the same payload, tag, logical-time, order, and
retraction semantics, making the callback check transplantable to another
Java, JNI, or native adapter without prescribing its scheduler.

`CallbackDeliveryObservation` adds the differential layer for multi-federate
streams.  The 2010 and 2025 fixture lanes feed two independently proxied
recipients, preserve sequence numbers within each recipient, and compare the
normalized semantic payload/provenance tuple.  Cross-recipient interleaving is
deliberately ignored because it is provider-defined; missing or duplicate
per-recipient sequence numbers still fail.  Provider-owned handle encodings
are decoded before comparison, so the same check can be transplanted to a
different RTI or native route.

The JNI callback-overload gates close the carrier-level gap beneath those
semantic vectors.  The 2010 lane invokes all 60 reflected Java callback
overloads for both logical-time families, and the 2025 lane invokes all 62.
Standard Java handles, typed sets/maps, enums, status arrays, information
sets, byte arrays, and logical times are converted into the corresponding
Python values and checked for payload preservation.  These tests deliberately
do not prescribe callback scheduling or federation policy, so the same matrix
can be transplanted to another Java, JNI, or native adapter.

These mixins are shared test infrastructure, not part of the customer-facing
`hla-rti-api` distribution. They live in the separate development-only
`umbra-rti-test-support` package because the native, JPype, and mock-Java
provider tests are separate packages.
...
| Scalar federation save success | Two real federates receive save initiation, begin/complete their saves, and each receives `federationSaved`; the real-JVM fixture and external IEEE-JAR JNI vector exercise the same lifecycle and enforce the federation-wide completion barrier; a mixed HLA_IMMEDIATE/HLA_EVOKED JNI vector additionally proves C++ notification ordering while the two standard Java callback routes retain independent delivery modes | Implemented |
...
| Native extendable-variant encoding | Native C++ exposes a provider-specific `HLAextendableVariantRecord` factory extension with copied mappings, length-prefixed alternatives, mapped decode, unknown-alternative skipping, and malformed length/padding/trailing rejection; the shared 15-case provider-extension matrix includes unknown empty alternatives, discriminator-padding corruption, truncated prefixes, zero/overlong mapped values, signed and positive length extremes, and trailing data; the Java JNI adapter consumes the standard creator through its provider-scoped Python façade and registers mappings on first `setVariant`, while the Java-shaped shared factory remains intentionally provider-neutral | Implemented in native C++ and the Java/JNI façade extension; excluded from the shared Python contract |
| Provider-owned unknown `DataElement` carrier | The 2010 and 2025 JNI probes return an implementation unknown to the standard encoder catalog and keep it as a generic Python façade; exact, empty, truncated, and overlong decode vectors plus exact-origin, offset-with-trailing, truncated, and empty `ByteWrapper` encode windows verify cursor/buffer preservation and typed `DecoderException`/`EncoderException` mapping | Implemented as bounded carrier/transport evidence; the fixed four-octet payload is test infrastructure, not an IEEE wire rule |
...
factory discovery, `umbra-rti-jpype` registers the `umbra-jni` entry-point
alias. Its `UmbraJniRtiFactory` preset only assembles the independently
supplied IEEE API JAR, Umbra bridge JAR, and native library before delegating
to the generic JPype adapter; the exact Java
`RtiFactoryFactory`/`ServiceLoader` remains the provider-selection boundary.
...
~~~powershell
$env:PYTHONPATH = 'packages/umbra-rti-api/src;packages/umbra-rti-test-support/src;packages/umbra-rti-jpype/src'
