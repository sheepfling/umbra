...
portable returned binary representation is therefore immutable `bytes`, while
read-only input arguments accept the shared `hla.rti1516_2025.BytesLike`
buffer contract. This is not a new public Python ownership model for the C++
`VariableLengthData` pointer.
