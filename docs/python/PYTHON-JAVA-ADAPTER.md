...
Umbra's own JNI bridge is not a separate Python distribution: the same
`umbra-rti-jpype` package provides `UmbraJniRtiFactory` and the `umbra-jni`
alias. It is a first-party configuration preset for the bridge/API/native
artifact trio, while vendor-specific adapters remain separate packages.
