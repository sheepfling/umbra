...
`compliance/default-region-requirements-contract.json`; the remaining
...
`docs/RELAXED-DDM-POLICY.md`. Broader relaxed-DDM matrices remain separate.
...
`compliance/whole-object-class-declaration-requirements-contract.json` and
`compliance/whole-object-class-declaration-api-contract.json` trace the two
...
`compliance/attribute-relevance-advisory-api-contract.json` pin the separate
...
`compliance/update-rate-value-api-contract.json` pin the FDD update-rate
...
`compliance/order-type-requirements-contract.json` pin the corresponding
...
`compliance/auto-provide-api-contract.json` separately trace the bounded Auto
...
`compliance/request-retraction-requirements-contract.json` captures the
...
and `compliance/timestamped-regional-attribute-update-api-contract.json` trace
...
`compliance/timestamped-directed-interaction-api-contract.json` trace the
...
`compliance/flush-queue-request-api-contract.json` trace the bounded
...
`compliance/asynchronous-delivery-api-contract.json` trace the bounded
...
`compliance/modify-lookahead-api-contract.json` trace the bounded Modify
...
`compliance/next-message-request-api-contract.json` trace the bounded Next
...
`compliance/timed-save-api-contract.json` trace the bounded timestamped
Request Federation Save overload. The registry stores one replaceable pending
request using official LogicalTime values, checks all current
time-constrained members, and releases the ordinary save callback fanout only
after queued and in-transit TSO payloads at or below the requested boundary
have completed. This is an in-memory embedded development slice; timed
