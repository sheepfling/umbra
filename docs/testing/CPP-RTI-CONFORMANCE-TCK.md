...
The following promoted slice, `cpp-tck.service-report-directed-interaction-declaration`
and its contract twin, also passed their focused installed-package gate in both
...
The next ordinary declaration-report slice, consisting of
`cpp-tck.service-report-interaction-publication`,
`cpp-tck.service-report-interaction-publication-contract`,
`cpp-tck.service-report-interaction-subscription`, and
`cpp-tck.service-report-interaction-subscription-contract`, passed an 8/8
focused installed-package run across both callback models. It covers ordinary
`PublishInteractionClass`, `UnpublishInteractionClass`,
`SubscribeInteractionClass`, and `UnsubscribeInteractionClass` reports,
including invalid-handle failures, typed Null returns, serial progression, and
the standard passive-subscription indicator. The source uses only official IEEE
C++ API headers and the standard library; provider, FOM, MIM, endpoint, and
...
- local deletion, receive-order deletion/removal, and resign-time object
  cleanup;
- receive-order object-removal delivery remains terminal for an object already
  discovered even when one subscriber withdraws before callback servicing,
  with immediate and evoked callback-model checks;
...
save/restore scenarios retain distinct IDs.
  ...
  `cpp-tck.callback-controls-object-name-reservation-failure` and its pure
  contract twin. It verifies contended reservation-failure callback suppression,
  ...
  callback-model cases.
  The newly promoted callback-control multiple-object-name slice adds
  `cpp-tck.callback-controls-multiple-object-name-reservation` and its pure
  contract twin. It verifies successful and mixed reservation callback sets,
  suppression while callbacks are disabled, release after re-enable,
  callback-model servicing, name cleanup, and standard federation lifecycle.
  Its focused installed-package lane passed 4/4 callback-model cases.
  ...
  The newly promoted callback-control attribute-relevance slice adds
  `cpp-tck.callback-controls-attribute-relevance-advisories` and its pure
  contract twin. It verifies unnamed and named per-object turn-updates-on and
  turn-updates-off callback suppression, release after re-enable, callback-model
  servicing, update-rate metadata, and declaration cleanup. Its focused
  installed-package lane passed 4/4 callback-model cases.
  The newly promoted callback-control Auto Provide slice adds
  `cpp-tck.callback-controls-auto-provide` and its pure contract twin. It
  verifies suppression and release of `provideAttributeValueUpdate`, callback-
  model servicing, object/attribute identity, the empty standard tag, the Auto
  Provide switch, and federation cleanup. Its focused installed-package lane
  passed 4/4 callback-model cases.
  The newly promoted callback-control ownership-assumption slice adds
  `cpp-tck.callback-controls-ownership-assumption` and its pure contract twin.
  It verifies unconditional divestiture, ownership-assumption callback
  suppression, release after re-enable, callback-model servicing, acquisition
  metadata and ownership state, and federation cleanup. Its focused
  installed-package lane passed 4/4 callback-model cases.
  The newly promoted callback-control ownership-unavailable slice adds
  `cpp-tck.callback-controls-ownership-unavailable` and its pure contract twin.
  It verifies If Available denial, ownership-unavailable callback suppression,
  release after re-enable, callback-model servicing, object/attribute/tag
  identity, unchanged ownership, and federation cleanup. Its focused
  installed-package lane passed 4/4 callback-model cases.
  The newly promoted callback-control ownership-release slice adds
  `cpp-tck.callback-controls-ownership-release-request` and its pure contract
  twin. It verifies regular acquisition, ownership-release request suppression,
  release after re-enable, callback-model servicing, acquisition and
  divestiture metadata, ownership handoff, and federation cleanup. Its focused
  installed-package lane passed 4/4 callback-model cases.
  The newly promoted callback-control ownership-cancellation slice adds
  `cpp-tck.callback-controls-ownership-acquisition-cancellation` and its pure
  contract twin. It verifies a pending regular acquisition, cancellation
  callback suppression, release after re-enable, callback-model servicing,
  request metadata, unchanged ownership, and federation cleanup. Its focused
  installed-package lane passed 4/4 callback-model cases.
  The newly promoted callback-control ownership-release-denial slice adds
  `cpp-tck.callback-controls-ownership-release-denied` and its pure contract
  twin. It verifies a pending regular acquisition, release-request metadata,
  denial callback suppression, release after re-enable, callback-model
  servicing, denial metadata, unchanged ownership, and federation cleanup. Its
  focused installed-package lane passed 4/4 callback-model cases.
  The newly promoted callback-control ownership-acquisition-notification slice
  adds `cpp-tck.callback-controls-ownership-acquisition-notification` and its
  ...
  verifies suppression and release of mixed ordinary ownership-query results,
  then exercises the distinct RTI-owned callback on a joined-federate MOM
  object using the adapter-supplied MIM and logical-time implementation. It
  checks exact object/attribute partitions, owning-federate metadata, callback
  servicing under both models, and federation cleanup. Its focused
  ...
  callback-model servicing, and federation cleanup. Its focused
  installed-package CTest lane passed all 4/4 cases in three repeats using only
  the official C++ API and standard library.
  The newly promoted callback-control time-advance slice adds
  `cpp-tck.callback-controls-time-advance` and its pure contract twin. It
  verifies time-regulation and time-constrained enablement, suppression and
  release of time-advance grants, callback-model servicing, exact grant times,
  and time-management/federation cleanup. Its focused installed-package CTest
   lane passed all 4/4 cases in three repeats using only the official C++ API and
   standard library.
   The newly promoted callback-control save/restore slice adds
   `cpp-tck.callback-controls-save-restore` and its pure contract twin. It
   verifies suppression and release of save initiation/completion, save-status,
   restore initiation/status, and restore-completion callbacks, exact labels and
   identity metadata, callback-model servicing, and federation cleanup. Its
   focused installed-package CTest lane passed all 4/4 cases in three repeats
   using only the official C++ API and standard library.
   The newly promoted callback-control save/restore-failure slice adds
   `cpp-tck.callback-controls-save-restore-failures` and its pure contract twin.
   It verifies suppression and release of `federationNotSaved` and
   ...
   and the time-advance grant, exact value/tag/time/order/transport/retraction
   metadata, reflection-before-grant ordering, callback-model servicing, and
   federation cleanup. Its focused installed-package CTest lane passed all 4/4
    cases in three repeats using only the official C++ API and standard library.
    ...
    twin. It verifies suppression and release of timestamped object removal and
    the time-advance grant, exact object/name/tag/time/order/retraction metadata,
    removal-before-grant ordering, callback-model servicing, and federation
    cleanup. Its focused installed-package CTest lane passed all 4/4 cases in
    three repeats using only the official C++ API and standard library.
    The newly promoted callback-control timestamped-retraction slice adds
    `cpp-tck.callback-controls-timestamped-retraction` and its pure contract
    twin. It verifies timestamped interaction delivery and retraction,
    suppression and release of the request-retraction callback, exact
    interaction and retraction-handle metadata, callback ordering,
    callback-model servicing, and federation cleanup. Its focused
    installed-package CTest lane passed all 4/4 cases in three repeats using
    ...
    transportation reports and confirmations, exact object/class/attribute/
    federate and transportation metadata, committed transportation state,
    callback-model servicing, and federation cleanup. Its focused
     installed-package CTest lane passed all 4/4 cases in three repeats using
     only the official C++ API and standard library.
     The newly promoted callback-control directed-interaction slice adds
     `cpp-tck.callback-controls-directed-interaction` and its pure contract twin.
     It verifies target routing, exact parameter/tag/transport/producer metadata,
     ...
     It verifies exact federation execution and member report content, the
      ...
      `cpp-tck.callback-controls-flush-queue-grant` and its pure contract twin.
      It verifies suppression and release of a queued timestamped interaction and
      `flushQueueGrant`, interaction-before-grant ordering, exact
      payload/tag/producer/time/order/retraction metadata, callback-model
      servicing, and time/federation cleanup. Its focused installed-package CTest
      lane passed all 4/4 cases in three repeats using only the official C++ API
      and standard library.
       ...
       interaction-before-grant ordering, exact timestamped interaction and
       retraction metadata, callback-model servicing, and time/federation cleanup.
       Its focused installed-package CTest lane passed all 4/4 cases in three
       repeats using only the official C++ API and standard library.
       The newly promoted callback-control time-role slice adds
       `cpp-tck.callback-controls-time-role-enablement` and its pure contract
       twin. It verifies suppression and release around standard time-regulation
       and time-constrained enablement callbacks, exact initial-time metadata,
...
callback-model cases.
The promoted ordinary multi-attribute Request Attribute Value Update slice adds
`cpp-tck.ordinary-multi-attribute-value-update-request-response` and its pure
contract twin. A requester asks for both attributes and then each selective
attribute set; the provider receives exact attribute projections and request
tags, and the ordinary response reflects exact values with response tags,
producer, and transportation metadata. Its focused lane passed 4/4
callback-model cases; the candidate-inclusive catalog-wide gate passed 808/810
cases, with the two expected adapter-managed connection-loss skips.
The promoted receive-order multi-attribute update slice adds
`cpp-tck.receive-order-multi-attribute-update-callback-cancellation` and its
pure contract twin. One ordinary two-attribute update is projected to a full
subscriber and a selective subscriber; `HLA_EVOKED` withdraws the selective
declaration before callback servicing and verifies no stale reflection, while
`HLA_IMMEDIATE` records synchronous delivery. A later update verifies the
withdrawn declaration remains inactive. Its focused lane passed 4/4
callback-model cases; the candidate-inclusive catalog-wide gate passed
812/814 cases, with the two expected adapter-managed connection-loss skips.
...
around regular and `If Available` ownership acquisition. They verify
unpublished/undefined acquisition failures, partial acquisition, overlapping
...
pure standard contract twin exercise adapter-supplied invalid additional FOM
modules, accept only standard FOM-validation exceptions, and verify that each
rejected join leaves membership and declaration lookup unchanged before a
...
The promoted `cpp-tck.object-class-lookup-lifecycle` scenario and its contract
twin passed 4/4 callback-model cases within the focused six-scenario lookup/FOM
regression lane, which passed 12/12 overall. They verify standard object-class
handle/name lifecycle boundaries, invalid-name/invalid-handle exceptions,
stable base handles, and shared extension object-class handles after an
adapter-supplied additional-FOM join. No provider-specific header or fixture
is used.
The promoted `cpp-tck.attribute-lookup-lifecycle` scenario and its contract
twin passed 4/4 callback-model cases within the focused eight-scenario
rich-FOM/lookup regression lane, which passed 16/16 overall. They verify
...
request, and complete a valid create/join/lookup/resign/destroy lifecycle.
Their focused installed-package lane passed 4/4 callback-model cases in
`.build\cpp-tck-fom-invalid-create-atomicity\invalid-create-atomicity-promoted.json`.
The promoted `cpp-tck.fom-invalid-composite-join-atomicity` scenario and its pure
standard contract twin exercise a mixed valid-plus-invalid additional-FOM join,
verify atomic rejection without membership or declaration mutation, and then
...
The promoted timed regular-candidate continuation contract twin exercises the
...
passed 4/4 callback-model cases; evoked mode retains negotiated continuation,
while immediate mode verifies the standard unavailable boundary and a
...
headers and the standard library; provider, dimensional FOM, endpoint,
  ...
  The promoted `cpp-tck.service-report-timestamped-directed-interaction`
  scenario and its contract twin extend the same MOM boundary through a
  time-regulated timestamped `SendDirectedInteraction`. The focused
  ...
  contract twin artifact, verifying typed service-report arguments, retraction
  identity, target routing, callback-before-grant ordering, and timestamp/order
  metadata. The source uses only the official IEEE C++ API, standard MIM data
  elements, and the standard library; the five-epsilon lookahead and all FOM,
  endpoint, provider, callback, and logical-time configuration remain
  adapter-owned.

  The promoted `cpp-tck.service-report-timestamped-attribute-update` scenario
  and its contract twin extend the same MOM boundary through a time-regulated
  timestamped `UpdateAttributeValues`. The focused installed-package lane
...
runner exposes the same queued-passel suppression, replacement-source delivery,
conveyed-region metadata, callback-before-grant, and cleanup surface as an
independently selectable pure C++ contract. Its focused base-and-contract lane
passed 4/4 callback-model cases; the promoted aggregate passed 706/706 CTest
cases with 706 direct passes and only the two expected adapter-managed
connection-loss skips in
`.build\cpp-tck-all\verified-evidence-timestamped-regional-interaction-subscription-replacement-contract.json`.
The source uses only official IEEE C++ headers and the standard library; the
dimensional FOM, provider, endpoint, callback, and logical-time configuration
remain adapter-owned.
...
and `cpp-tck.timestamped-regional-interaction-tar-nmr-contract` runners expose
those two timestamped regional interaction boundaries as independently
selectable pure C++ contracts. Their focused base-and-contract lane passed 8/8
callback-model cases; the promoted aggregate passed 710/710 CTest cases with
710 direct passes and only the two expected adapter-managed connection-loss
skips in
`.build\cpp-tck-all\verified-evidence-timestamped-regional-interaction-source-resignation-tar-nmr-contracts.json`.
...
`cpp-tck.timestamped-regional-attribute-alternate-advances-contract` runners
expose the timestamped regional Update/Reflect and alternate-advance boundaries
as independently selectable pure C++ contracts. Their focused base-and-contract
lane passed 8/8 callback-model cases; the promoted aggregate passed 714/714
CTest cases with 714 direct passes and only the two expected adapter-managed
connection-loss skips in
`.build\\cpp-tck-all\\verified-evidence-timestamped-regional-attribute-update-alternate-contracts.json`.
The source uses only official IEEE C++ headers and the standard library;
dimensional FOM, provider, endpoint, callback, and logical-time configuration
remain adapter-owned.
...
`cpp-tck.timestamped-regional-attribute-source-resignation-contract` runners
expose source-association replacement, changed-lookahead Time Regulation
re-enable, and post-resignation delivery as independently selectable pure C++
contracts. Their focused base-and-contract lane passed 12/12 callback-model
cases; the promoted aggregate passed 720/720 CTest cases with 720 direct passes
and only the two expected adapter-managed connection-loss skips in
...
`cpp-tck.timestamped-default-region-attribute-mixed-fanout-contract` runners
expose alternate advances, Time Constrained and Time Regulation re-enable, and
mixed-fanout behavior as independently selectable pure C++ contracts. Their
focused base-and-contract lane passed 16/16 callback-model cases; the promoted
aggregate passed 728/728 CTest cases with 728 direct passes and only the two
expected adapter-managed connection-loss skips in
...
runners add independently selectable pure C++ contracts for baseline delivery,
...
ownership contract-twin lane passed 8/8 callback-model cases in
`.build\cpp-tck-all\verified-evidence-standard-ownership-cancellation-continuation.json`; the focused
