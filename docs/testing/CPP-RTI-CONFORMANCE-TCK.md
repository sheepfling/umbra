...
The verified lane currently runs 340 promoted scenarios (680 callback-model
...
and callback
configuration. The default `--scenario-set verified` selects the 340 catalog
entries with `promotion=promoted`, which produces 680 cases with
`--callback-model both`.
After that gate is green, pass `--scenario-set all` to include the later
adapter-required entries; the current catalog contains 350 IDs and 700 cases,
