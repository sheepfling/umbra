...
The verified lane currently runs 347 promoted scenarios (694 callback-model
...
and callback
configuration. The default `--scenario-set verified` selects the 347 catalog
entries with `promotion=promoted`, which produces 694 cases with
`--callback-model both`.
After that gate is green, pass `--scenario-set all` to include the later
adapter-required entries; the current catalog contains 357 IDs and 714 cases,
