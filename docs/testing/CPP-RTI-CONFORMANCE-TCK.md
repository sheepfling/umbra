...
- zero-dimensional, partial, wrong-context, foreign-region, and in-use region
  ...
  The verified lane currently runs 459 promoted scenarios (918 callback-model
...
and callback
  configuration. The default `--scenario-set verified` selects the 459 catalog
  entries with `promotion=promoted`, which produces 918 cases with
`--callback-model both`.
After that gate is green, pass `--scenario-set all` to include the later
  adapter-required entries; the current catalog contains 459 IDs and 918 cases,
