...
The verified lane currently runs 348 promoted scenarios (696 callback-model
...
and callback
configuration. The default `--scenario-set verified` selects the 348 catalog
entries with `promotion=promoted`, which produces 696 cases with
`--callback-model both`.
After that gate is green, pass `--scenario-set all` to include the later
adapter-required entries; the current catalog contains 358 IDs and 716 cases,
