...
The verified lane currently runs 399 promoted scenarios (798 callback-model
...
connection-loss skips, plus 796 direct passes and the same two skips. The
...
and callback
configuration. The default `--scenario-set verified` selects the 399 catalog
entries with `promotion=promoted`, which produces 798 cases with
`--callback-model both`.
After that gate is green, pass `--scenario-set all` to include the later
adapter-required entries; the current catalog contains 399 IDs and 798 cases,
