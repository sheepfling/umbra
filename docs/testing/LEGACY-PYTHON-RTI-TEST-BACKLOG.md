...
[FOM stress-corpus backlog](../fom/FOM-STRESS-CORPUS-BACKLOG.md) separately records
...
| `SB-TIME-007` | Partially implemented, TSO integration | `tests/scenarios/test_time_management_federation.py`; `packages/hla-verification/src/hla/verification/section8_matrix.py` | The bounded C++ coverage now includes both the earlier mixed-family case and focused three-member object-attribute cases: a timestamp-7 update is submitted before a timestamp-5 update plus a second timestamp-5 cohort member, and independent constrained recipients receive the complete equal-timestamp cohort before timestamp 7 without imposing a tie-break under direct TAR and under TARA/NMRA. Retain transport-arrival, cross-process, and remaining alternate-advance variants as follow-on fixtures; their ordering expectations must still be specified independently for Umbra. |
| `SB-TIME-008` | Partially implemented, save/restore phase | `tests/test_rti1516_2025_python1516_2025_runtime.py`; sibling time-window proof families | The bounded timestamped save boundary, source-linked untimed and timestamped time-constrained save-admission paths, and process-local untimed restore now exist in the official C++ profile. A two-federate Catch2 case proves a TSO interaction at the exact TAR save timestamp is delivered before direct Initiate Federate Save and the matching grant; a three-member case proves two constrained members are admitted before a non-time-constrained member; TARA and NMRA cases prove equal versus strictly-later save boundaries; and an NMR case proves direct initiation at the matching timestamp. A three-member TARA/NMRA case proves both strict ordinary forms are ready before non-time-constrained notification. A dedicated FQR case proves an equal actual Flush Queue Grant leaves a timestamped save pending while a later FQG follows queued TSO and direct initiation; mixed FQR/TAR cases prove readiness whether FQR or TAR dispatches first. A six-member scenario now combines TAR, NMR, TARA, NMRA, and FQR before non-time-constrained notification. A cross-member TSO case proves a delayed constrained federate's queued interaction still precedes its direct initiation after another constrained member begins saving. C++-native cases also prove a post-save retraction designator cannot alias fresh traffic, a live interaction/ledger returns after rollback, a terminal tombstone retains its classification, a saved logical-time/actual-lookahead window is restored after post-save mutation, and one deferred lookahead decrease target survives rollback. Paired HLA_EVOKED/HLA_IMMEDIATE one-federate NRG cases capture an ordinary TAR while direct save initiation is in progress, then restore it after post-save time work and verify that a fresh grant follows Federation Restored; paired one-federate FQR cases prove the special grant path has the same restored-before-grant ordering under both callback models; and a six-scenario single-federate matrix proves saved-pending TARA/NMR/NMRA restores under both callback models. Add multi-member pending-advance, queued/in-transit TSO, role/resignation churn, future-exclusion, broader family, durable snapshot, and transport restore stress only after the corresponding C++ state boundaries are specified. |
...
multi-member pending-advance or queued/in-transit restore, broader message-family, durable-snapshot, and
transport-restore stress after their C++ state boundaries are specified.

**2026-08-24 update:** the two-member HLA_EVOKED save/restore regression now
captures two constrained pending TAR requests in one process-local image and
proves independent restored grants. The older SB-TIME-008 intake sentence
above and its 2026-08-18 follow-up predate this case; multi-member pending
advance is no longer an untested intake item. Queued/in-transit restore,
role/resignation churn, future-exclusion, broader message families, durable
snapshots, and transport restore remain open.
...
A matching default-source/default-region interaction regression now keeps the
same queued TSO passel for a regional time-constrained recipient across that
producer regulation transition and proves the supplied-empty callback marker
and callback-before-grant ordering. Changed-lookahead, alternate-advance, and
the other timestamped families remain backlog work.
A focused ordinary timestamped-attribute companion now keeps one queued
passel across Time-Constrained disable/re-enable and proves one reflection
before the matching grant, including its payload and retraction metadata.
...
A paired timed default-region interaction regression now crosses a logical-time
six save boundary with a timestamp-eight private-default passel, restores it,
and verifies Flush Queue delivery plus Request Retraction. It is intentionally
kept beside the timed attribute case as focused C++ evidence; durable and
general timed restore remain backlog work.
...
time-window backlog, separate from the now-evidenced paired HLA_EVOKED/
HLA_IMMEDIATE one-federate NRG pending-advance restore and the TARA/NMR/NMRA
matrix, and not evidence for
broader time-constrained-state, durable
snapshots, or transport recovery. A companion case saves actual
...
The sibling's `timestamped_update_survives_ownership_transfer` stress shape is
now translated into the native C++ case
`Embedded accepted timestamped attribute update survives ownership transfer
before constrained grant`. It queues an accepted TSO attribute passel, performs
an official unconditional divestiture and If Available acquisition before the
recipient's grant, and verifies the original producer, payload, tag,
timestamp/order, and retraction handle at `Reflect Attribute Values`. This is a
bounded in-flight ownership seam only; the complete ownership/resignation,
alternate-advance, save/restore, regional, transport, and cross-process
matrices remain backlog work.
...
| ID | Priority and phase | Sibling source | Umbra intake outcome |
| --- | --- | --- | --- |
| `SB-MOM-001` | Partially implemented, first file-sink service-report slice | `tests/mom/test_mom_catalog_validation_v012.py`; `tests/verification/test_mom_observer_slice_v013.py`; `tests/backends/test_python_backend_federation_extended.py` | Reuse the subject/witness/enable/drain topology only. Umbra now proves the file-selected arm for seven successful-void support services: six Boolean setters (the four relevance/scope advisory setters, `Set Convey Region Designator Sets Switch`, and `Set Exception Reporting Switch`) plus `Set Automatic Resign Directive`. The advisory setters append exact Table 5 `[null]` records with serials zero through three, the Convey and automatic-resign setters each prove an exact record, and the Exception Reporting case appends exactly one Table 5 `[null]` file record while suppressing a callback to an otherwise matching subscriber. A new focused C++ case also proves the reporting-disabled arm: with the official Restaurant FOM's Service Reporting switch disabled and the file switch explicitly enabled, an accepted `Get Dimension Handle` leaves the preallocated file unchanged and delivers no `HLAreportServiceInvocation` callback. The existing ordinary `Send Interaction` MOM case covers the interaction-selected arm. The sibling's JSONL audit sink and its 2010-oriented outcome rules are not portable semantics. RL-042 still blocks generic return/failure record mapping; RL-043 remains open for RTI-originated producer mapping and the broader interaction-delivery matrix. |
| `SB-MOM-002` | Partially implemented, bounded report success/failure matrix | `tests/mom/test_mom_catalog_validation_v012.py`; `tests/backends/test_python_backend_federation_extended.py`; `tests/backends/test_python_backend_object_ownership_extended.py` | The C++ failure slice now covers the seven §10.6--§10.12 known-object, object-instance, attribute, and update-rate lookups: each preserves its supplied argument form, Null returned argument, false success indicator, exception text, and serial ordering in focused filesystem and HLA_IMMEDIATE public-interaction matrices. The native MOM interaction-count lane now also rejects a malformed `HLAsetSwitches` `HLAresignAction` value before the accepted-service boundary and proves that `HLAinteractionsSent` remains zero, so a failed adjustment cannot become a positive count. Continue the rejected-adjustment/no-positive-report shape and declaration, federation, and ownership family sweeps as a checklist. For every admitted wrapper, independently map supplied arguments, returned argument, declared exceptions, serial allocation, and callback cleanup to 2025 sources. Do not inherit the sibling's service names, exception encoding, or report-count assumptions. RL-152 records the Requirements Lab's missing conditional failure mapping. |
| `SB-MOM-003` | Partially implemented, bounded public MOM-object route | `tests/verification/test_mom_observer_slice_v013.py`; `tests/backends/test_python_backend_time_ddm_extended.py` | Umbra retains a registry-owned joined-federate MOM snapshot with a common object identity, the static initial values (including the real report-file path), an immutable private federate point, switch-stable lifetime, resignation removal, and rejoin replacement. The C++ public route now proves observer discovery/reflection, direct requested-value delivery, callback-time withdrawal, and two-federate isolation with distinct static paths in both callback models. The broader periodic/interaction surface remains open. `HLAreportServiceFile` publication still carries the Table 8/MIM timing conflict (RL-041) and unresolved §6.9 producer-designator mapping for RTI-registered objects (RL-043); the test intentionally records the RTI-owned path and default-invalid producer result, and does not model the sibling's audit sink. |
...
## DDM and region-lifecycle scenarios

| ID | Priority and phase | Sibling source | Umbra intake outcome |
| --- | --- | --- | --- |
| `SB-DDM-001` | **Implemented**, empty `Create Region` characterization | `tests/backends/test_python_backend_time_ddm_extended.py::test_create_region_and_commit_region_modifications_reject_not_connected_not_joined_invalid_region_and_save_restore` | The sibling test treats an empty dimension set as `InvalidRegionContext`. The official IEEE 1516.1-2025 C++ declaration lists only `InvalidDimensionHandle` and lifecycle/save exceptions, and the 9.2 preconditions do not require a non-empty set. Umbra therefore keeps the standards-correct zero-dimensional region-template path and the native Catch2 case creates, commits, queries, and deletes it. The sibling exception is retained as a portability note, not a C++ conformance rule; broader region realization, association, overlap, save/restore, remote transport, and package evidence remain open. |

## FOM and stress-corpus candidates

The detailed [FOM stress-corpus backlog](../fom/FOM-STRESS-CORPUS-BACKLOG.md) is the
...
ordinary non-regional timestamped-attribute Time-Constrained disable/re-enable
case, plus one process-local saved-time-window rollback and a deferred-lookahead
rollback, paired HLA_EVOKED/HLA_IMMEDIATE saved-pending NRG TAR restores, paired
HLA_EVOKED/HLA_IMMEDIATE saved-pending FQR restores, and a paired-model TARA/NMR/NMRA matrix
are now evidenced. A bounded three-member non-regional timestamped-attribute
restore case also restores one queued passel to two constrained recipients,
proving independent Flush Queue delivery and Request Retraction; broader
multi-member pending/in-transit and terminal-lifetime restore matrices remain
open. A one-member timestamped-attribute terminal-tombstone restore case now
preserves the saved `MessageCanNoLongerBeRetracted` classification and rejects
the discarded post-save handle. A matching timestamped-object-deletion case
also restores the saved terminal classification after payload/object/name
reclamation, rejects a distinct post-save deletion handle, and keeps the
released name absent; terminal-lifetime matrices for the other payload
families remain open. A bounded two-member timestamped-directed interaction
companion now preserves a terminal designator across restore alongside a
separate live admission passel and rejects a distinct post-save directed
handle; callback delivery and live-payload recovery remain explicit follow-ons.
The matching explicit-source regional-interaction case now also preserves its
terminal designator across restore with an overlap-qualified admission passel
and rejects a distinct post-save regional handle; regional callback and live
payload recovery remain explicit follow-ons. A matching explicit-source
regional-attribute case now preserves a terminal timestamped Update Attribute
Values classification across restore with a separate overlap-qualified
admission passel and rejects a distinct post-save regional-attribute handle;
regional callback and live-payload recovery remain explicit follow-ons. A
matching default-source regional-attribute case now preserves the same terminal
classification across restore with a separate private-default admission passel
and rejects a distinct post-save default-region handle; callback and live-
payload recovery remain explicit follow-ons. A matching default-source
regional-interaction case now preserves a terminal timestamped Send Interaction
classification across restore with a separate private-default admission passel
and rejects a distinct post-save default-region interaction handle; callback and
live-payload recovery remain explicit follow-ons. The next milestone is broader
re-enable and terminal-lifetime restore,
then remaining message families and uncovered deletion shapes.
