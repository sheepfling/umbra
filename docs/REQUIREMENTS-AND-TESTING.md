...
attributes, that the attribute and representation type match. The two standard
instance identifier names remain a separately reviewed boundary. Directed
...
Create/Destroy/Join/Resign, active-federate name/handle lookup,
...
MessageRetractionHandle decoding. The separate
...
fields, sender exclusion, and terminal post-delivery retraction.
...
bound removal before the grant, and the timestamped callback fields.
Directed or regional forms, alternate advance modes,
request-retraction callbacks, transport, and conformance remain outside these
slices.
...
sets to the opt-in adapter and an active-membership Catch2 selector. These
lookups operate only within the caller's joined federation. The Lab has no
higher-level implementation mapping for either service, so this remains API
traceability—not catalog or evidence promotion.
...
idempotent removal, and resign cleanup. The Lab has no higher-level
...
limited receive-order interaction path; it does not claim declaration advisory
callbacks, MOM behavior, or conformance.
...
active/passive subscription, class-local removal, and resign cleanup; the
...
candidate for that 5.3.3(f) rule. The contracts still exclude advisory behavior,
full ownership, update-rate enforcement, regions, catalog evidence, and
conformance.
...
generated names, source-known lookup, exact and superclass discovery, late
...
subscription, and no-time overlap-routing slice; timestamped regional sends,
...
sends` Catch2 case. The case covers independent ordinary/regional state,
...
updates by overlap` Catch2 case. The case covers no-name registration with
explicit associations, additive/idempotent association, empty-region no-ops,
regional subscriptions and unsubscriptions, committed region ownership and
dimension validation, overlap-filtered discovery and no-time reflection, the
optional sent-region callback marker, and disjoint-range suppression. Named
regional registration, default-region synthesis,
timestamped/retraction behavior, broader DDM routing, save/restore, packaging,
...
discovers one known regional object, and moves committed subscriber-region
overlap and the producer's update-region association through both HLA_IMMEDIATE
and HLA_EVOKED delivery. It verifies one grouped callback per transition and
...
only; default-region synthesis, timestamped/retraction behavior, broader DDM
...
multi-transport FOM passels, known-class projection, passive subscriptions,
...
traceability—not catalog or evidence promotion. The limited receive-order
interaction and attribute-update paths apply FOM-selected mandatory types;
they still do not claim custom transportation types, timestamped/local
...
boundaries, publication, exact and passive superclass subscriptions, one
callback for a dual subscription, sender exclusion, tag/producer/mandatory
...
transportation propagation. Universal-subscription semantics, timestamped/retraction behavior,
directed DDM, ordering, FOM sharing-policy enforcement, catalog evidence,
...
The bounded timestamped interaction, attribute-update, and object-deletion
contracts are the current public TSO traffic consumers; these time-advance contracts still cover
only TAR and do not imply support for other advance modes. The Lab exports no higher-level
...
timestamped queue state; the three timestamped-service contracts record the
...
queued/in-flight list reports from outliving Disconnect. It still has no
connection-lost or other federation-event callbacks, complete object/ownership
behavior, or package support. Its joined federates can obtain a caller-owned factory for
their stored selected reference-time representation through `getTimeFactory`.
They can also resolve active federate names and handles within their joined
federation; an invalid handle and a valid handle outside that federation take
their distinct declared exception paths. They can resolve object- and
...
subscribed class for each recipient, projects available parameters, suppresses
...
set; committed region ranges and the 2025 overlap rule gate no-time regional
sends, empty sent sets suppress delivery, and queued callbacks recheck the
overlap. It also supports unnamed non-region object registration, generated
...
sharing policy, save/restore, and resign-action object disposition remain
...
unowned conditions; the offer itself does not establish ownership. The profile
does not yet run a continuing owner search after later join, discovery, or
publication changes.
...
federates; the limited interaction and attribute-update paths apply the
FOM-selected mandatory type. Custom transportation and general message
transport remain unimplemented. Joined federates start at the selected initial
...
timestamped-interaction contract adds only the non-regional timestamped Send
Interaction path; other timestamped service families, alternate advance modes,
and general time-management support remain unimplemented.
