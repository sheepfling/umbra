...
The promoted `cpp-tck.default-region-interaction-routing` slice and its pure
standard contract twin cover ordinary interaction routing across explicit
...
The promoted `cpp-tck.regional-interaction-empty-subscription-sets` slice and
its pure standard contract twin verify that empty regional subscription and
unsubscription sets are no-ops, while a corresponding non-empty
unsubscription removes the route. Their focused installed-package lane
passed 4/4 callback-model cases. The portable source remains limited to the
...
The promoted `cpp-tck.ordinary-multi-attribute-subscription-projection` slice
and its pure standard contract twin project a two-attribute ordinary update to
both-, first-only-, and second-only subscribers, then verify selective updates,
delivery metadata, and owner loopback exclusion. Their focused lane passed 4/4
callback-model cases; its preceding catalog-wide gate passed 804/806 cases,
with the two expected adapter-managed connection-loss skips. The promoted
`cpp-tck.ordinary-multi-attribute-value-update-request-response` slice and its
pure standard contract twin request both attributes and then each selective
attribute set, verify exact provider projections and request tags, and reflect
exact response values with response tags, producer, and transportation metadata.
...
passed 4/4 callback-model cases in
  ...
  The promoted `cpp-tck.service-report-timestamped-directed-interaction` and
  `cpp-tck.service-report-timestamped-directed-interaction-contract` runners
  extend that boundary to time-regulated timestamped directed interactions.
...
The promoted
`cpp-tck.service-report-timestamped-delete-object-instance` and
`cpp-tck.service-report-timestamped-delete-object-instance-contract` runners
extend that boundary to time-regulated timestamped `DeleteObjectInstance`.
Their focused installed-package lane passed 4/4 callback-model cases in
...
ordering using only the official IEEE C++ API, standard MIM data elements, and
...
adapter-owned.
The promoted `cpp-tck.inherited-object-attribute-projection-contract` runner
adds the independently selectable standard inherited object-attribute
projection contract. Its focused base-and-contract lane passed 4/4
callback-model cases; the source uses only official IEEE C++ headers and the
standard library, while the rich model FOM, provider, endpoint, callback, and
...
official C++ API.
The promoted `cpp-tck.fom-additional-module-join-atomicity` case and its
contract twin reject adapter-supplied invalid additional FOM modules with
standard exceptions, verify no membership or declaration mutation after each
...
The promoted `cpp-tck.object-class-lookup-lifecycle` case and its contract twin
verify standard object-class handle/name lookup before connection, before
membership, for invalid inputs, and after additional-FOM composition. The new
pair passed 4/4 callback-model cases inside the focused six-scenario lane,
which passed 12/12 overall using only official C++ API headers and the standard
library.
The promoted `cpp-tck.attribute-lookup-lifecycle` case and its contract twin
verify standard attribute handle/name lookup before connection, before
membership, across inherited base/derived object classes, for invalid inputs,
and across federates. The new pair passed 4/4 callback-model cases inside the
focused eight-scenario rich-FOM/lookup lane, which passed 16/16 overall using
only official C++ API headers and the standard library.
The promoted `cpp-tck.parameter-lookup-lifecycle` case and its contract twin
verify standard parameter handle/name lookup before connection, before
membership, across inherited base/derived interaction classes, for invalid
inputs, and across federates. The new pair passed 4/4 callback-model cases
inside the focused ten-scenario rich-FOM/lookup lane, which passed 20/20 overall
using only official C++ API headers and the standard library.
The promoted `cpp-tck.dimension-lookup-lifecycle` case and its contract twin
verify standard dimension handle/name/upper-bound and available-dimension lookup
before connection, before membership, after resign, for invalid inputs, and
across federates. The pair passed 4/4 callback-model cases in its focused
installed-package lane using only official C++ API headers and the standard
library; the dimensional FOM and dimension names remain adapter inputs.
...
Their focused installed-package lane passed 4/4 callback-model cases using only
the official C++ API and standard library.
The promoted `cpp-tck.fom-invalid-composite-join-atomicity` case and its
contract twin exercise mixed valid-plus-invalid additional-FOM join rejection,
...
The promoted `cpp-tck.fom-invalid-composite-mim-create-atomicity` case and its
contract twin reject a mixed valid-plus-invalid FOM module vector atomically
when creating with the standard MIM, then reuse the same federation name for a
valid FOM/MIM create, join, lookup, resign, and destroy lifecycle. Their focused
installed-package CTest lane passed 4/4 callback-model cases using only the
official C++ API and standard library.
The promoted `cpp-tck.fom-empty-mim-create-atomicity` case and its contract twin
reject an empty FOM module vector with standard `InvalidFOM`, then reuse the same
federation name for a valid FOM/MIM create, join, lookup, resign, and destroy
lifecycle. Their focused installed-package CTest lane passed 4/4 callback-model
cases using only the official C++ API and standard library.
...
The promoted `cpp-tck.callback-controls-object-removal` and
`cpp-tck.callback-controls-object-removal-contract` runners extend the same
standard callback gate to ordinary object removal; their focused installed-
package lane passed 4/4 callback-model cases.
The promoted `cpp-tck.callback-controls-object-discovery` and
`cpp-tck.callback-controls-object-discovery-contract` runners extend the same
standard callback gate to ordinary object discovery; their focused installed-
package lane passed 4/4 callback-model cases.
The promoted `cpp-tck.callback-controls-object-name-reservation` and
`cpp-tck.callback-controls-object-name-reservation-contract` runners extend the
same standard callback gate to ordinary object-name reservation; their focused
installed-package lane passed 4/4 callback-model cases.
The promoted `cpp-tck.callback-controls-multiple-object-name-reservation` and
`cpp-tck.callback-controls-multiple-object-name-reservation-contract` runners
extend the same standard callback gate to successful and mixed multiple
object-name reservations; their focused installed-package lane passed 4/4
...
focused installed-package lane passed 4/4 callback-model cases.
The promoted `cpp-tck.callback-controls-ownership-unavailable` and
`cpp-tck.callback-controls-ownership-unavailable-contract` runners extend the
same standard callback gate to the If Available denial callback; their focused
installed-package lane passed 4/4 callback-model cases.
The promoted `cpp-tck.callback-controls-ownership-release-request` and
`cpp-tck.callback-controls-ownership-release-request-contract` runners extend
the same standard callback gate to ownership release requests and acquisition;
...
and `cpp-tck.callback-controls-ownership-acquisition-cancellation-contract`
runners extend the same standard callback gate to ownership-acquisition
cancellation; their focused installed-package lane passed 4/4 callback-model
cases.
The promoted `cpp-tck.callback-controls-ownership-release-denied` and
`cpp-tck.callback-controls-ownership-release-denied-contract` runners extend
the same standard callback gate to ownership-release denial and the resulting
ownership-unavailable callback; their focused installed-package lane passed 4/4
callback-model cases.
The promoted `cpp-tck.callback-controls-ownership-acquisition-notification` and
`cpp-tck.callback-controls-ownership-acquisition-notification-contract` runners
...
cases.
  ...
  The promoted `cpp-tck.callback-controls-synchronization` and
  `cpp-tck.callback-controls-synchronization-contract` runners extend the same
  standard callback gate to synchronization-point announcements and Federation
  Synchronized completion; their focused installed-package lane passed all 4/4
  callback-model cases in three repeats.
  The promoted `cpp-tck.callback-controls-time-advance` and
  `cpp-tck.callback-controls-time-advance-contract` runners extend the same
  standard callback gate to time-regulation, time-constrained, and time-advance
  grant callbacks. Their focused installed-package lane passed all 4/4
  callback-model cases in three repeats.
  The promoted `cpp-tck.callback-controls-save-restore` and
  `cpp-tck.callback-controls-save-restore-contract` runners extend the same
  standard callback gate to federation save/restore lifecycle and status
  callbacks. Their focused installed-package lane passed all 4/4 callback-model
  cases in three repeats.
  The promoted `cpp-tck.callback-controls-save-restore-failures` and
  `cpp-tck.callback-controls-save-restore-failures-contract` runners extend the
  same standard callback gate to federation save and restore failure callbacks
  and exact failure reasons. Their focused installed-package lane passed all
  4/4 callback-model cases in three repeats.
  The promoted `cpp-tck.callback-controls-restore-request-failure` and
  `cpp-tck.callback-controls-restore-request-failure-contract` runners extend
  the same standard callback gate to failed restore requests, exact save-label
  delivery, and suppression of restore-start callbacks. Their focused
  installed-package lane passed all 4/4 callback-model cases in three repeats.
  The promoted `cpp-tck.callback-controls-timestamped-attribute-update` and
  `cpp-tck.callback-controls-timestamped-attribute-update-contract` runners
  extend the same standard callback gate to timestamped attribute reflection and
  its time-advance grant. Their focused installed-package lane passed all 4/4
  callback-model cases in three repeats.
  The promoted `cpp-tck.callback-controls-timestamped-object-removal` and
  `cpp-tck.callback-controls-timestamped-object-removal-contract` runners
  extend the same standard callback gate to timestamped object removal and its
  time-advance grant. Their focused installed-package lane passed all 4/4
  callback-model cases in three repeats.
  The promoted `cpp-tck.callback-controls-timestamped-retraction` and
  `cpp-tck.callback-controls-timestamped-retraction-contract` runners extend
  the same standard callback gate to timestamped interaction retraction and
  request-retraction callbacks. Their focused installed-package lane passed
  all 4/4 callback-model cases in three repeats.
  The promoted `cpp-tck.callback-controls-transportation` and
  `cpp-tck.callback-controls-transportation-contract` runners extend the same
  standard callback gate to attribute and interaction transportation reports
  and confirmations. Their focused installed-package lane passed all 4/4
  callback-model cases in three repeats.
  The promoted `cpp-tck.callback-controls-directed-interaction` and
  `cpp-tck.callback-controls-directed-interaction-contract` runners extend the
  same standard callback gate to directed-interaction target routing, exact
  parameter/tag/transport/producer metadata, callback suppression/release, and
  federation cleanup. Their focused installed-package lane passed all 4/4
  callback-model cases in three repeats.
  The promoted `cpp-tck.callback-controls-federation-reports` and
  `cpp-tck.callback-controls-federation-reports-contract` runners extend the
  same standard callback gate to exact federation execution/member report
  content and the federation-does-not-exist report, including suppression,
  release, callback-model servicing, and cleanup. Their focused installed-
  package lane passed all 4/4 callback-model cases in three repeats.
  The promoted `cpp-tck.callback-controls-attribute-scope-advisories` and
  `cpp-tck.callback-controls-attribute-scope-advisories-contract` runners
  extend the same standard callback gate to regional `attributesInScope` and
  `attributesOutOfScope` transitions, exact object/attribute metadata, explicit
  advisory-switch setup, suppression, release, and region cleanup. Their focused
  installed-package lane passed all 4/4 callback-model cases in three repeats.
  The promoted `cpp-tck.callback-controls-flush-queue-grant` and
  `cpp-tck.callback-controls-flush-queue-grant-contract` runners extend the same
  standard callback gate to queued timestamped interaction delivery and
  `flushQueueGrant`, including suppression/release, interaction-before-grant
  ordering, exact time/retraction metadata, callback-model servicing, and time
  cleanup. Their focused installed-package lane passed all 4/4 callback-model
  cases in three repeats.
   The promoted `cpp-tck.callback-controls-available-time-advance-callbacks` and
   `cpp-tck.callback-controls-available-time-advance-callbacks-contract` runners
   extend the same standard callback gate to Time Advance Request Available and
   Next Message Request Available delivery, interaction-before-grant ordering,
   exact timestamped interaction/retraction metadata, callback-model servicing,
   and cleanup. Their focused installed-package lane passed all 4/4 callback-model
   cases in three repeats.
   The promoted `cpp-tck.callback-controls-time-role-enablement` and
   `cpp-tck.callback-controls-time-role-enablement-contract` runners extend the
   same standard callback gate to time-regulation and time-constrained enablement
   callbacks, exact initial-time metadata, callback-model servicing, and cleanup.
   Their focused installed-package lane passed all 4/4 callback-model cases in
   three repeats.
...
`cpp-tck.timestamped-local-delete-attribute-contract` runners expose standard
recipient-local timestamped deletion boundaries as independently selectable
pure C++ contracts, with provider, FOM, endpoint, logical-time, and callback
configuration supplied by the adapter.
