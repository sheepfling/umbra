...
[[nodiscard]] std::unique_ptr<DataElement> cloneElement(DataElement const& element) {
  ...
  return std::unique_ptr<DataElement>(clone.release());
}
...
std::auto_ptr<DataElement> HLAvariantRecord::clone() const {
  return std::auto_ptr<DataElement>(new HLAvariantRecord(*this));
}
...
void HLAvariantRecord::decode(VariableLengthData const& value) {
...
void HLAvariantRecord::addVariant(
    DataElement const& discriminant,
...
void HLAvariantRecord::addVariantPointer(
    DataElement const& discriminant,
...
void HLAvariantRecord::setVariant(
    DataElement const& discriminant,
...
void HLAvariantRecord::setVariantPointer(
    DataElement const& discriminant,
    DataElement* valuePtr) {
  if (valuePtr == nullptr) {
    invalidEncoding(L"An HLAvariantRecord variant pointer must be non-null.");
