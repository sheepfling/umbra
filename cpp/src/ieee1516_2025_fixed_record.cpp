...
void appendClone(HLAfixedRecordImplementation& implementation, DataElement const& element) {
  ...
  implementation.elements.emplace_back(std::move(clone));
}
...
std::unique_ptr<DataElement> HLAfixedRecord::clone() const {
  return std::make_unique<HLAfixedRecord>(*this);
}
...
HLAfixedRecord& HLAfixedRecord::decode(VariableLengthData const& value) {
...
HLAfixedRecord& HLAfixedRecord::appendElement(DataElement const& dataElement) {
  appendClone(*_impl, dataElement);
  return *this;
}

HLAfixedRecord& HLAfixedRecord::appendElementPointer(DataElement* dataElement) {
...
HLAfixedRecord& HLAfixedRecord::set(std::size_t index, DataElement const& dataElement) {
...
HLAfixedRecord& HLAfixedRecord::setElementPointer(std::size_t index, DataElement* dataElement) {
  if (dataElement == nullptr) {
    invalidEncoding(L"An HLAfixedRecord element pointer must be non-null.");
  }
  if (index >= _impl->elements.size() || _impl->elements[index].element == nullptr) {
