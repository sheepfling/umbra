...
bool DimensionHandleDirectory::reconcile(FomCatalog const* catalog) {
  if (catalog == nullptr) {
    return true;
  }

  for (std::string const& name : catalog->dimensionNames()) {
    if (name.empty()) {
      return false;
    }
...
std::optional<std::uint64_t> DimensionHandleDirectory::handleFor(
    std::string const& dimensionName) const {
  auto const found = handlesByName_.find(dimensionName);
  return found == handlesByName_.end() ? std::nullopt : std::optional{found->second};
}

std::optional<std::string> DimensionHandleDirectory::nameFor(std::uint64_t handle) const {
  if (handle == 0) {
    return std::nullopt;
  }
