...
// types. The bounded non-void DDM slice additionally uses DimensionHandleSet
// and RangeBounds, whose values remain tied to the official MIM enumerators.
...
// Table 5 specifies DimensionHandleSet as Array<DimensionHandle>. The
// official C++ set has deterministic handle order, which is retained here.
[[nodiscard]] std::wstring formatMomDimensionHandleSet(
...
// Table 5 specifies ParameterHandle as a String containing the exact value
// returned by the handle's standard toString() operation.
[[nodiscard]] std::wstring formatMomParameterHandle(
...
// Table 5 specifies RangeBounds as Record<lower:Number, upper:Number>.
[[nodiscard]] std::wstring formatMomRangeBounds(
...
// Table 5 renders ServiceGroup as the quoted public enumeration spelling.
// Keep this closed mapping separate from the MIM numeric service-type value so
// service-report arguments retain the source-defined indicator names.
[[nodiscard]] std::wstring formatMomServiceGroup(
    rti1516_2025::ServiceGroup serviceGroup);
...
// MIM interaction encoding. ServiceReportRecord uses the same argument fields
// for supplied arguments; its returned-argument wrapper is kept separate below
// because the rendered Table 5 example wraps the return value in an array.
...
// Table 5 explicitly gives [null] as the returned-argument representation for
// a successful void service. Keep this formatter stable for the existing void
// service-report lane.
...
// ServiceReportRecord's non-void returned argument follows the rendered 2025
// Table 5 ReturnArgument example: one HLAreturnedArgument record in an array.
// The public HLAreportServiceInvocation interaction continues to use the MIM's
// fixed HLAargument parameter encoding via encodeMomServiceInvocation().
[[nodiscard]] std::wstring formatMomSuccessfulServiceReportRecord(
    std::uint32_t serialNumber,
    std::wstring const& service,
    std::vector<MomServiceArgument> const& suppliedArguments,
    MomServiceArgument const& returnedArgument);
// A failed service invocation still has a ServiceReportRecord. IEEE
// 1516.1-2025 §11.5.1 requires the returned argument to use the Null
// HLAargumentType when HLAsuccessIndicator is false, while HLAexception holds
// the textual exception description.
[[nodiscard]] std::wstring formatMomFailedServiceReportRecord(
    std::uint32_t serialNumber,
    std::wstring const& service,
    std::vector<MomServiceArgument> const& suppliedArguments,
