...
constexpr char kReportInteractionsSentInteractionClassName[] =
    "HLAinteractionRoot.HLAmanager.HLAfederate.HLAreport.HLAreportInteractionsSent";
constexpr char kReportDirectedInteractionsSentInteractionClassName[] =
    "HLAinteractionRoot.HLAmanager.HLAfederate.HLAreport.HLAreportDirectedInteractionsSent";
...
constexpr char kReportNumberOfClassesParameterName[] = "HLAnumberOfClasses";
constexpr char kReportObjectClassParameterName[] = "HLAobjectClass";
...
constexpr std::uint64_t kMomInteractionsSentEndpointRegionHandle =
    std::numeric_limits<std::uint64_t>::max() - 7U;
constexpr std::uint64_t kMomDirectedInteractionsSentEndpointRegionHandle =
...
EmbeddedFederationRegistry::planPendingNegotiatedAttributeOwnershipDivestitureConfirmations(
  ...
  std::map<std::tuple<std::uint64_t, bool, std::uint64_t>, std::set<std::uint64_t>>
  ...
  for (auto& [key, attributeHandles] : attributesByDivesterAndRequest) {
    auto const [divestingFederateId, candidateIfAvailable, acquisitionRequestId] = key;
...
MomInteractionsSentReportPlan
EmbeddedFederationRegistry::planMomInteractionsSentReport(
    std::wstring const& federationName,
    std::uint64_t requestingFederateId,
    std::uint64_t reportedFederateId) const {
  auto instrumentationScope = beginInstrumentation(
  ...
  std::scoped_lock lock(mutex_);
  MomInteractionsSentReportPlan result;
  result.reportedFederateId = reportedFederateId;
  auto const federation = federations_.find(federationName);
  if (federation == federations_.end()) {
    result.status = MomObjectInstanceCountsReportStatus::federation_does_not_exist;
  ...
  auto const reportClassHandle = federation->second.interactionClassHandles->handleFor(
      kReportInteractionsSentInteractionClassName);
  auto const federateDimensionHandle = federation->second.dimensionHandles->handleFor(
      kHlaFederateDimensionName);
  auto const transportationParameterHandle = federation->second.parameterHandles->handleFor(
      federation->second.definition.catalog.get(),
  ...
  // HLAreportInteractionsSent requires one response for each supported
  // transportation type, including an empty HLAinteractionCounts NULL
  // response when the represented federate has no sends in that bucket.
  ...
  for (auto const& [interactionClassHandle, byTransportation] :
       reportedMember->second.successfulInteractionCountsByClassAndTransportation) {
  ...
  result.routing.endpointRegionHandle = kMomInteractionsSentEndpointRegionHandle;
  result.routing.endpointRegion = {
      {*federateDimensionHandle},
      {{*federateDimensionHandle, {normalizedFederate, normalizedFederate + 1U}}},
      true,
...
std::optional<ReceiveOrderInteractionRecipient>
EmbeddedFederationRegistry::momInteractionsSentReportRecipientFor(
    std::wstring const& federationName,
    std::uint64_t reportedFederateId,
    std::uint64_t receivingFederateId) const {
  std::scoped_lock lock(mutex_);
  ...
  std::map<std::uint64_t, RegionSpecificationSnapshot> const endpointOverride{
      {kMomInteractionsSentEndpointRegionHandle, endpointRegion},
  };
  std::set<std::uint64_t> const endpointRegionHandles{
      kMomInteractionsSentEndpointRegionHandle,
  };
  return candidateReceiveOrderInteractionRecipient(
      federation->second,
      InteractionProducer::rti(),
...
MomDirectedInteractionsSentReportPlan
EmbeddedFederationRegistry::planMomDirectedInteractionsSentReport(
    std::wstring const& federationName,
    std::uint64_t requestingFederateId,
    std::uint64_t reportedFederateId) const {
  auto instrumentationScope = beginInstrumentation(
  ...
  std::scoped_lock lock(mutex_);
  MomDirectedInteractionsSentReportPlan result;
  result.reportedFederateId = reportedFederateId;
  auto const federation = federations_.find(federationName);
  if (federation == federations_.end()) {
    result.status = MomObjectInstanceCountsReportStatus::federation_does_not_exist;
  ...
  auto const reportClassHandle = federation->second.interactionClassHandles->handleFor(
      kReportDirectedInteractionsSentInteractionClassName);
  auto const federateDimensionHandle = federation->second.dimensionHandles->handleFor(
      kHlaFederateDimensionName);
  auto const transportationParameterHandle = federation->second.parameterHandles->handleFor(
      federation->second.definition.catalog.get(),
  ...
  // HLAreportDirectedInteractionsSent has the same per-transport NULL
  // response rule, but reads only the accepted directed-send ledger.
  result.interactionClassCountsByTransportation.try_emplace("HLAreliable");
  result.interactionClassCountsByTransportation.try_emplace("HLAbestEffort");
  for (auto const& [interactionClassHandle, byTransportation] :
       reportedMember->second.successfulDirectedInteractionCountsByClassAndTransportation) {
    if (interactionClassHandle == 0U) {
      continue;
    }
    for (auto const& [transportationName, count] : byTransportation) {
  ...
  result.routing.transportationParameterHandle = *transportationParameterHandle;
  result.routing.interactionCountsParameterHandle = *interactionCountsParameterHandle;
  result.routing.endpointRegionHandle = kMomDirectedInteractionsSentEndpointRegionHandle;
  result.routing.endpointRegion = {
      {*federateDimensionHandle},
      {{*federateDimensionHandle, {normalizedFederate, normalizedFederate + 1U}}},
      true,
...
std::optional<ReceiveOrderInteractionRecipient>
EmbeddedFederationRegistry::momDirectedInteractionsSentReportRecipientFor(
    std::wstring const& federationName,
    std::uint64_t reportedFederateId,
    std::uint64_t receivingFederateId) const {
  std::scoped_lock lock(mutex_);
  ...
  std::map<std::uint64_t, RegionSpecificationSnapshot> const endpointOverride{
      {kMomDirectedInteractionsSentEndpointRegionHandle, endpointRegion},
  };
  std::set<std::uint64_t> const endpointRegionHandles{
      kMomDirectedInteractionsSentEndpointRegionHandle,
  };
  return candidateReceiveOrderInteractionRecipient(
      federation->second,
      InteractionProducer::rti(),
