...
constexpr std::size_t kRegionHandleEncodedLength = 8;

std::uint64_t handleValue(RegionHandleImplementation const* implementation);

std::array<unsigned char, kRegionHandleEncodedLength> encodeValue(std::uint64_t value) {
  std::array<unsigned char, kRegionHandleEncodedLength> encoded{};
...
std::uint64_t decodeValue(VariableLengthData const& encodedValue) {
  if (encodedValue.size() != kRegionHandleEncodedLength || encodedValue.data() == nullptr) {
    throw CouldNotDecode(L"An Umbra RegionHandle must contain exactly eight bytes.");
  }

  auto const* bytes = static_cast<unsigned char const*>(encodedValue.data());
  std::uint64_t value = 0;
  for (unsigned char byte : std::array<unsigned char, kRegionHandleEncodedLength>{
...
size_t RegionHandle::encode(void* buffer, size_t bufferSize) const {
  if (buffer == nullptr || bufferSize < kRegionHandleEncodedLength) {
    throw CouldNotEncode(L"The RegionHandle output buffer must contain at least eight bytes.");
  }
