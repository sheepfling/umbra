...
#error "The timed save/restore negotiated ownership cancellation test requires the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"timed-live-tso-regional-attribute-multi-recipient-negotiated-cancel-pending-ownership-" +
...
class ReportingFederateAmbassador final
 ...
 public:
  ...
  void requestDivestitureConfirmation(
      ...
      VariableLengthData const& userSuppliedTag) override {
    ++divestitureConfirmationCallbackCount;
    callbackOrder.push_back("divestiture-confirmation");
    (void)objectInstance;
    (void)releasedAttributes;
    (void)userSuppliedTag;
  }
  ...
  std::size_t divestitureConfirmationCallbackCount = 0U;
};
...
TEST_CASE(
    "Embedded timed multi-recipient regional timestamped attribute update survives requester cancellation of pending negotiated ownership transfer after restore",
    ...
    "[cancel-pending-ownership-acquisitions][negotiated-attribute-ownership-divestiture]"
    "[attribute-ownership-acquisition-if-available][willing-to-acquire]"
    "[timestamped-regional-attribute-update][explicit-source]"
    "[mixed-fanout][tso-regional-attribute-update-timed-cancel-state]"
    "[tso-regional-attribute-update-timed-if-available-state]"
    ...
    "[tso-regional-attribute-update-timed-negotiated-cancel-pending-after-restore]"
  ...
  std::wstring const saveLabel =
      L"restore-live-explicit-source-regional-negotiated-cancel-pending-ownership";
  ...
  // Create a real regular acquisition after restore.  The owner-release
  // callback is queued on the publisher, but requester resignation must
  // consume it as stale work.
  REQUIRE_NOTHROW(firstReceiver->publishObjectClassAttributes(
      firstReceiverSoda,
      firstReceiverFlavorOnly));
  unsigned char const acquisitionTagBytes[] = {0x41, 0x43, 0x51, 0x2D, 0x54};
  ...
  unsigned char const negotiatedTagBytes[] = {0x4E, 0x45, 0x47, 0x2D, 0x43};
  ...
  REQUIRE(publisherReports.divestitureConfirmationCallbackCount == 0U);
