...
#error "The ordinary regional interaction MOM-failure test requires the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"regional-interaction-failure-mom-report-" +
...
TEST_CASE(
    "Embedded service reporting delivers failed regional Send Interaction With Regions invocations through MOM interaction",
    "[integration][development-profile][federation-management][interaction-management]"
    "[ddm][mom][service-reporting][service-report-interaction][service-failure]"
    "[ordinary-regional-interaction-failure][ordinary-regional-interaction-failure-mom-interaction]"
    "[rti.service.ordinary-regional-interaction-failure-matrix-interaction]"
  ...
  auto const unrelatedRegion = publisher->createRegion(
      rti1516_2025::DimensionHandleSet{unrelatedDimension});
  REQUIRE_NOTHROW(publisher->setRangeBounds(
      unrelatedRegion,
      unrelatedDimension,
      rti1516_2025::RangeBounds(0UL, 4UL)));
  REQUIRE_NOTHROW(publisher->commitRegionModifications(
      RegionHandleSet{unrelatedRegion}));
  ...
  auto const regionSetValue = L"[\"" + region.toString() + L"\"]";
  auto const invalidRegionSetValue =
      L"[\"" + RegionHandle{}.toString() + L"\"]";
  auto const unrelatedRegionSetValue =
      L"[\"" + unrelatedRegion.toString() + L"\"]";
  ...
  verifyFailure(
      ...
      parameterMapValue,
      regionSetValue,
      L"InteractionClassNotDefined: Send Interaction With Regions requires a defined InteractionClassHandle.");
  ...
  // IEEE 1516.1-2025 §9.1.3.3: an associated region may only contain
  // dimensions available to the sent interaction class.
  REQUIRE_THROWS_AS(
      publisher->sendInteractionWithRegions(
          interactionClass,
          ParameterHandleValueMap{{parameter, parameterValue}},
  ...
  verifyFailure(
      ...
      validInteractionValue,
      parameterMapValue,
      unrelatedRegionSetValue,
      L"InvalidRegionContext: The supplied region dimensions are not available for this interaction class.");

  REQUIRE_NOTHROW(publisher->setServiceReportingSwitch(false));
  REQUIRE_NOTHROW(publisher->setSendServiceReportsToFileSwitch(false));
  REQUIRE_NOTHROW(observer->unsubscribeInteractionClass(reportClass));
  REQUIRE_NOTHROW(publisher->deleteRegion(unrelatedRegion));
  REQUIRE_NOTHROW(publisher->deleteRegion(region));
  REQUIRE_NOTHROW(observer->resignFederationExecution(CANCEL_THEN_DELETE_THEN_DIVEST));
