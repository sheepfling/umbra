...
#error "The joined-federate MOM deleted-object-count periodic tests require the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"federation-joined-mom-deleted-object-count-periodic-" +
...
TEST_CASE(
    ...
    "[request-attribute-value-update][delete-object-instance][send-interaction]"
    "[reflect-attribute-values][joined-federate-mom-deleted-object-count-periodic]"
  ...
  observer->joinFederationExecution(
      L"mom-deleted-object-count-periodic-observer", L"observer", federationName);
  ...
  auto const deletedInstancesAttribute = observer->getAttributeHandle(
      momClass, L"HLAobjectInstancesDeleted");
  ...
  REQUIRE(deletedInstancesAttribute.isValid());
  REQUIRE(reliable.isValid());
  REQUIRE_NOTHROW(observer->subscribeObjectClassAttributes(
      momClass,
      AttributeHandleSet{federateHandleAttribute, deletedInstancesAttribute},
      true));

  auto const ownerFederate = owner->joinFederationExecution(
      L"mom-deleted-object-count-periodic-owner", L"owner", federationName);
  ...
  auto requestDeletedCount = [&] {
    ...
    auto const reflectionIterator = std::find_if(
        ...
        [&](ReportingFederateAmbassador::AttributeReflectionReport const& report) {
          return report.objectInstance == ownerMomObject &&
              report.attributeValues.contains(deletedInstancesAttribute);
        });
    ...
    REQUIRE_NOTHROW(decoded.decode(
        reflection.attributeValues.at(deletedInstancesAttribute)));
    return decoded.get();
  };

  REQUIRE(requestDeletedCount() == 0);
  ...
  drainCallbacks(*observer);
  auto const periodicBeforeDeletion = std::find_if(
      observerReports.attributeReflectionReports.begin() +
  ...
  REQUIRE(periodicBeforeDeletion != observerReports.attributeReflectionReports.end());
  REQUIRE(periodicBeforeDeletion->attributeValues.size() == 1U);
  REQUIRE(periodicBeforeDeletion->transportationType == reliable);
  REQUIRE_FALSE(periodicBeforeDeletion->producingFederate.isValid());
  REQUIRE(periodicBeforeDeletion->userSuppliedTag.size() == 0U);
  HLAinteger32BE periodicZero;
  REQUIRE_NOTHROW(periodicZero.decode(
      periodicBeforeDeletion->attributeValues.at(deletedInstancesAttribute)));
  ...
  REQUIRE_NOTHROW(observer->sendInteraction(
      ...
      ParameterHandleValueMap{
          ...
          {periodParameter, HLAinteger32BE{1}.encode()}},
      VariableLengthData{}));
  auto const beforePostDeletionPeriodic = observerReports.attributeReflectionReports.size();
  std::this_thread::sleep_for(std::chrono::milliseconds(1100));
  drainCallbacks(*observer);
  auto const periodicAfterDeletion = std::find_if(
      observerReports.attributeReflectionReports.begin() +
          static_cast<std::ptrdiff_t>(beforePostDeletionPeriodic),
      observerReports.attributeReflectionReports.end(),
      [&](ReportingFederateAmbassador::AttributeReflectionReport const& report) {
        return report.objectInstance == ownerMomObject &&
            report.attributeValues.contains(deletedInstancesAttribute);
      });
  REQUIRE(periodicAfterDeletion != observerReports.attributeReflectionReports.end());
  REQUIRE(periodicAfterDeletion->attributeValues.size() == 1U);
  REQUIRE(periodicAfterDeletion->transportationType == reliable);
  REQUIRE_FALSE(periodicAfterDeletion->producingFederate.isValid());
  REQUIRE(periodicAfterDeletion->userSuppliedTag.size() == 0U);
  HLAinteger32BE periodicTwo;
  REQUIRE_NOTHROW(periodicTwo.decode(
      periodicAfterDeletion->attributeValues.at(deletedInstancesAttribute)));
