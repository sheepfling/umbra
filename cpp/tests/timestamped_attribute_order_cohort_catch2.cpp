...
#error "The timestamped attribute-order cohort test requires the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"timestamped-attribute-order-cohort-" +
...
TEST_CASE(
    "Embedded timestamped attribute updates preserve different-timestamp order for each constrained recipient",
    "[integration][development-profile][object-management][time-management]"
    "[timestamped-attribute-update][timestamped-attribute-order-cohort][tso][multi-federate-callback-ordering]"
  ...
  // Both constrained recipients request the exact timestamp-5 boundary.
  // Each receives the complete equal-timestamp cohort before its grant.
  ...
  // A second request then admits the distinct timestamp-7 record, retaining
  // callback-before-grant ordering for each recipient.
  REQUIRE_NOTHROW(tara->timeAdvanceRequest(
      rti1516_2025::HLAinteger64Time(7)));
  REQUIRE_NOTHROW(nmra->timeAdvanceRequest(
      rti1516_2025::HLAinteger64Time(7)));
