...
#error "The negotiated resign-action test requires the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"resign-action-cancel-negotiated-pending-" +
...
class ReportingFederateAmbassador final
 ...
 public:
  ...
  struct DivestitureConfirmation final {
    ObjectInstanceHandle objectInstance;
    AttributeHandleSet attributes;
    VariableLengthData userSuppliedTag;
  };
  ...
  void requestDivestitureConfirmation(
      ...
      VariableLengthData const& userSuppliedTag) override {
    divestitureConfirmations.push_back({
        objectInstance,
        releasedAttributes,
        userSuppliedTag,
    });
  ...
  std::vector<DivestitureConfirmation> divestitureConfirmations;
};
...
TEST_CASE(
    "Standalone resign action cancels a pending negotiated ownership transfer for the departing acquirer",
    "[integration][development-profile][federation-management][ownership-management]"
    "[resign-action-cancel-negotiated-pending][standalone][2025]"
  ...
  // A regular acquisition first queues an owner-side release request. The
  // negotiated divestiture selects that same pending transfer for confirmation.
  ...
  REQUIRE(ownerReports.divestitureConfirmations.empty());
  ...
  // Directive 3 cancels both pending requester-side states. Neither stale
  // owner callback may be delivered after the requester has resigned.
