...
TEST_CASE(
    "Embedded live regional timestamped attribute update survives source mutation and resignation after restore",
    "[integration][development-profile][federation-management][save-restore]"
    "[object-management][ddm][time-management][tso][resignation]"
    "[timestamped-regional-attribute-update][explicit-source]"
    "[tso-regional-attribute-update-live-resignation-state]"
  ...
  auto const federationName =
      std::wstring{L"live-tso-regional-attribute-source-resignation-restore"};
  ...
  // Reach a callback-capable constrained boundary without delivering the
  // timestamp-7 passel. The save image must capture that still-live explicit
  // source association and its queued TSO payload.
  REQUIRE_NOTHROW(receiver->timeAdvanceRequest(
      rti1516_2025::HLAinteger64Time(1)));
  REQUIRE(receiverReports.timeAdvanceGrantValues.empty());
  auto const saveLabel = std::wstring{L"live-regional-source-resignation-save"};
  ...
  // The independent clock advances the federation frontier after the source
  // has left, allowing the surviving constrained receiver to flush its saved
  // regional passel.
  ...
  REQUIRE(clockReports.timeAdvanceGrantValues.front().value == L"2");
  ...
  // The independent regulator advanced to 2 with a lookahead interval of 5,
  // so the restored receiver's federation-wide flush frontier is 7.
