...
ScopedTemporaryDirectory temporaryServiceReportDirectory() {
  ...
  for (std::size_t attempt = 0U; attempt != 1024U; ++attempt) {
    auto const path = parent /
        ("umbra-timestamped-delete-report-" +
...
std::wstring nextFederationName() {
  ...
  return L"timestamped-delete-report-" +
...
TEST_CASE(
    "Embedded service reporting records timestamped Delete Object Instance before removal callback",
    "[integration][development-profile][federation-management][object-management][time-management]"
    "[mom][service-report-file][service-reporting][timestamped-delete-object-instance][tso]"
    "[timestamped-delete-object-instance-service-report][timestamped-delete-object-instance-sender-file]"
    "[rti.service.delete-object-instance][federate.callback.remove-object-instance]") {
  ...
  REQUIRE_NOTHROW(publisher->joinFederationExecution(
      L"timestamped-delete-report-publisher", L"publisher", federationName));
  REQUIRE_NOTHROW(receiver->joinFederationExecution(
      L"timestamped-delete-report-receiver", L"subscriber", federationName));
  ...
  REQUIRE_NOTHROW(publisher->deleteObjectInstance(objectInstance, tag, timestamp));
  ...
  auto const expectedRecord = std::string{
      R"({"HLAserialNumber":0,"HLAreturnedArgument":[{"HLAargumentType":34,"HLAargumentName":"","HLAargumentValue":null}],"HLAservice":"DeleteObjectInstance","HLAsuppliedArguments":[{"HLAargumentType":37,"HLAargumentName":"Object instance designator","HLAargumentValue":")" +
      objectInstanceValue +
