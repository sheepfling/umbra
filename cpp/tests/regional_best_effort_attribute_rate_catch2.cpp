...
#error "The regional best-effort-rate test requires the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"regional-best-effort-attribute-rate-" +
...
TEST_CASE(
    "Embedded regional best-effort attribute updates honor the subscribed rate",
    "[integration][development-profile][federation-management][object-management][ddm]"
    "[regional-attribute-update][update-rate-reduction][receive-order][attribute-update]"
  ...
  REQUIRE_NOTHROW(publisher->joinFederationExecution(
      L"regional-rate-publisher", L"publisher", federationName));
  REQUIRE_NOTHROW(receiver->joinFederationExecution(
      L"regional-rate-subscriber", L"subscriber", federationName));
  ...
  REQUIRE_NOTHROW(publisher->publishObjectClassAttributes(
      publisherClass,
      AttributeHandleSet{publisherValue}));
  ...
  auto const subscriptionRegion = receiver->createRegion(
      DimensionHandleSet{receiverX, receiverY});
  ...
  auto sendAndDrain = [&](unsigned char value) {
  ...
  REQUIRE(receiverReports.attributeReflectionReports.front().attributeValues.contains(
      receiverValue));
  sendAndDrain(0x22);
  // The immediate second update remains within the FDD Low (0.2 Hz) interval;
  // a regional subscription must use that same rate gate as an ordinary one.
  REQUIRE(receiverReports.attributeReflectionReports.size() == 1U);

  // Removing and re-establishing the regional declaration starts a fresh
  // subscription generation.  The first update in that new epoch must not be
  // suppressed by the old wall-clock admission history.
