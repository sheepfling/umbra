...
#error "The save/restore default-region attribute tests require the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"restore-live-tso-default-region-attribute-" +
...
TEST_CASE(
    "Embedded federation restore restores a saved live timestamped default-region attribute update",
    "[integration][development-profile][federation-management][save-restore]"
    "[object-management][ddm][time-management][tso]"
    "[timestamped-default-region-attribute-update][default-region][timestamped-default-region-attribute-restore]"
  ...
  unsigned char const valueBytes[] = {0x55, 0x4E, 0x54, 0x2D, 0x41, 0x54, 0x54};
  unsigned char const tagBytes[] = {0x55, 0x4E, 0x54, 0x2D, 0x54, 0x53, 0x4F};
  VariableLengthData const value(valueBytes, sizeof(valueBytes));
  VariableLengthData const tag(tagBytes, sizeof(tagBytes));
  std::wstring const saveLabel = L"restore-live-default-region-attribute-save";
  ...
  REQUIRE_NOTHROW(publisherHandle = publisher->joinFederationExecution(
      L"restore-live-default-region-attribute-publisher",
      L"publisher",
      federationName));
  REQUIRE_NOTHROW(receiver->joinFederationExecution(
      L"restore-live-default-region-attribute-receiver",
      L"subscriber",
      federationName));
  ...
  auto const receiverFlavor = receiver->getAttributeHandle(
      receiverSoda,
      fixture_hla::fixture::flavor);
  auto const receiverDimension = receiver->getDimensionHandle(
