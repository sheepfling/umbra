...
#error "The joined-federate MOM updated-object-count periodic tests require the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"federation-joined-mom-updated-object-count-periodic-" +
...
TEST_CASE(
    "Embedded joined-federate MOM exposes HLAobjectInstancesUpdated count",
    ...
    "[reflect-attribute-values][joined-federate-mom-updated-object-count-periodic]"
  ...
  auto const observerFederate = observer->joinFederationExecution(
      L"mom-updated-object-count-periodic-observer", L"observer", federationName);
  ...
  auto const ownerFederate = owner->joinFederationExecution(
      L"mom-updated-object-count-periodic-owner", L"owner", federationName);
  ...
  auto requestMomCounts = [&] {
    ...
    REQUIRE_NOTHROW(observer->requestAttributeValueUpdate(
        ownerMomObject,
        AttributeHandleSet{updatedInstancesAttribute, updatesSentAttribute},
        VariableLengthData{}));
    drainCallbacks(*observer);
    ...
    HLAinteger32BE updatedInstances;
    HLAinteger32BE updatesSent;
    REQUIRE_NOTHROW(updatedInstances.decode(
        reflection.attributeValues.at(updatedInstancesAttribute)));
    REQUIRE_NOTHROW(updatesSent.decode(
        reflection.attributeValues.at(updatesSentAttribute)));
    return std::pair<std::int32_t, std::int32_t>{
        updatedInstances.get(), updatesSent.get()};
  };

  // The direct MOM request is valid before any application object update.
  ...
  REQUIRE(counts.second == 1);

  // Repeated accepted service invocations for one object do not inflate the
  // distinct-object HLAobjectInstancesUpdated count.
  REQUIRE_NOTHROW(owner->updateAttributeValues(
      firstObject,
  ...
  REQUIRE_NOTHROW(owner->updateAttributeValues(
      ...
      VariableLengthData{}));
  counts = requestMomCounts();
  REQUIRE(counts.first == 2);
  REQUIRE(counts.second == 3);
  ...
  HLAinteger32BE periodicUpdatedInstances;
  HLAinteger32BE periodicUpdatesSent;
  REQUIRE_NOTHROW(periodicUpdatedInstances.decode(
      periodicReflection->attributeValues.at(updatedInstancesAttribute)));
  REQUIRE_NOTHROW(periodicUpdatesSent.decode(
      periodicReflection->attributeValues.at(updatesSentAttribute)));
  REQUIRE(periodicUpdatedInstances.get() == 2);
  REQUIRE(periodicUpdatesSent.get() == 3);
