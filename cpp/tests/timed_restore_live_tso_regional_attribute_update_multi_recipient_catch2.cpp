...
TEST_CASE(
    "Embedded timed federation restore restores one queued explicit-source regional timestamped attribute update to multiple recipients",
    ...
    "[timestamped-regional-attribute-timed-restore-multi-recipient]"
  ...
  std::wstring const saveLabel =
      L"restore-live-explicit-source-regional-tso-multi";
  ...
  REQUIRE_NOTHROW(publisherHandle = publisher->joinFederationExecution(
      L"restore-live-tso-regional-multi-publisher",
      L"publisher",
      federationName));
  REQUIRE_NOTHROW(firstReceiver->joinFederationExecution(
      L"restore-live-tso-regional-multi-first",
      L"subscriber",
      federationName));
  REQUIRE_NOTHROW(secondReceiver->joinFederationExecution(
      L"restore-live-tso-regional-multi-second",
      L"subscriber",
      federationName));
  ...
  // Terminalize the live designator and remove the live association. The
  // restore image must put the saved source region, both recipient-local
  // entries, and the shared retraction ledger back.
  ...
  REQUIRE_NOTHROW(secondReceiver->flushQueueRequest(
      rti1516_2025::HLAinteger64Time(10)));
  REQUIRE_FALSE(secondReceiver->evokeCallback(0.0));
  verifyReflection(firstReceiverReports, firstReceiverFlavor, firstReceiverRegion);
  verifyReflection(
      secondReceiverReports, secondReceiverFlavor, secondReceiverRegion);
