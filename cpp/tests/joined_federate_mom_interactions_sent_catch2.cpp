...
#error "The joined-federate MOM interactions-sent tests require the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"federation-joined-mom-interactions-sent-" +
...
std::vector<DecodedInteractionCount> decodeInteractionCounts(
  ...
  // HLAinteractionCount fixed records. Decode the nested list with the
  // standard data-element classes rather than a private byte formatter.
...
TEST_CASE(
    "Embedded MOM requestInteractionsSent reports class and transportation counts",
    ...
    "[ddm][callback-model][joined-federate-mom-interactions-sent]"
  ...
  REQUIRE_NOTHROW(requester->joinFederationExecution(
      L"mom-interactions-sent-requester", L"requester", federationName));
  ...
  REQUIRE_NOTHROW(senderFederate = sender->joinFederationExecution(
      L"mom-interactions-sent-sender", L"sender", federationName));
  FederateHandle idleFederate;
  REQUIRE_NOTHROW(idleFederate = idle->joinFederationExecution(
      L"mom-interactions-sent-idle", L"idle", federationName));
  ...
  auto const mainCourseServed = sender->getInteractionClassHandle(
      L"HLAinteractionRoot.CustomerTransactions.FoodServed.MainCourseServed");
  auto const temperatureOk = sender->getParameterHandle(mainCourseServed, L"TemperatureOk");
  auto const accuracyOk = sender->getParameterHandle(mainCourseServed, L"AccuracyOk");
  auto const timelinessOk = sender->getParameterHandle(mainCourseServed, L"TimelinessOk");
  auto const serverId = sender->getDimensionHandle(L"ServerId");
  ...
  REQUIRE(accuracyOk.isValid());
  REQUIRE(timelinessOk.isValid());
  ...
  REQUIRE_NOTHROW(sender->publishInteractionClass(mainCourseServed));
  ...
  auto const senderRegion = sender->createRegion(DimensionHandleSet{serverId});
  REQUIRE_NOTHROW(sender->setRangeBounds(senderRegion, serverId, RangeBounds(1UL, 2UL)));
  REQUIRE_NOTHROW(sender->commitRegionModifications(RegionHandleSet{senderRegion}));
  HLAfixedRecord serviceStat;
  serviceStat.appendElement(HLAboolean{true})
      .appendElement(HLAboolean{true})
      .appendElement(HLAboolean{true});
  REQUIRE_NOTHROW(sender->sendInteractionWithRegions(
      mainCourseServed,
      ParameterHandleValueMap{
          {temperatureOk, serviceStat.encode()},
          {accuracyOk, serviceStat.encode()},
          {timelinessOk, HLAboolean{true}.encode()}},
      RegionHandleSet{senderRegion},
      VariableLengthData{}));
  ...
  verifyReport(
      ...
      reliable,
      {{takeOrder, 1}, {mainCourseServed, 1}});
