...
#error "The failed Local Delete Object Instance service-report test requires the Umbra source directory."
#endif
...
ScopedTemporaryDirectory temporaryServiceReportDirectory() {
  ...
  for (std::size_t attempt = 0U; attempt != 1024U; ++attempt) {
    auto const path = parent /
        ("umbra-local-delete-failure-report-" + std::to_string(++counter) +
...
std::wstring nextFederationName() {
  ...
  return L"local-delete-object-instance-failure-report-" +
...
TEST_CASE(
    "Embedded service reporting records failed local-delete object-instance invocations",
    ...
    "[local-delete-failure][rti.service.local-delete-failure-matrix]") {
  ...
  REQUIRE_NOTHROW(owner->joinFederationExecution(
      L"local-delete-failure-owner", L"owner", federationName));
  REQUIRE_NOTHROW(requester->joinFederationExecution(
      L"local-delete-failure-requester", L"requester", federationName));
  ...
  auto const reportRecord = [](std::uint32_t serial,
    ...
    return std::string{"{\"HLAserialNumber\":"} + std::to_string(serial) +
        ...
        R"(","HLAsuppliedArguments":[{"HLAargumentType":37,"HLAargumentName":"Object instance designator","HLAargumentValue":")" +
        objectValue + R"("}],"HLAsuccessIndicator":)" +
        (success ? "true" : "false") + R"(,"HLAexception":)" +
        (exception.empty() ? std::string{"null"}
  ...
  auto const failedUnknown = reportRecord(
      0U,
      "LocalDeleteObjectInstance",
      invalidObjectValue,
      false,
      "ObjectInstanceNotKnown: Local Delete Object Instance requires a known ObjectInstanceHandle.");
  REQUIRE_THROWS_AS(
      requester->localDeleteObjectInstance(ObjectInstanceHandle{}),
      rti1516_2025::ObjectInstanceNotKnown);
  REQUIRE(readTextFile(reportFile) == initialText + failedUnknown);

  auto const successfulDelete = reportRecord(
      1U,
      "LocalDeleteObjectInstance",
      validObjectValue,
      true,
  ...
  REQUIRE(readTextFile(reportFile) == initialText + failedUnknown + successfulDelete);

  auto const failedDeleted = reportRecord(
      2U,
      "LocalDeleteObjectInstance",
      validObjectValue,
      false,
  ...
  REQUIRE(readTextFile(reportFile) ==
          initialText + failedUnknown + successfulDelete + failedDeleted);
