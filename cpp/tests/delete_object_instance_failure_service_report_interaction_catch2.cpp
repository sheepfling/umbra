...
#error "The receive-order Delete Object Instance MOM failure test requires the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"delete-object-instance-mom-failure-" +
...
TEST_CASE(
    "Embedded service reporting delivers failed receive-order Delete Object Instance invocations through MOM interaction",
    ...
    "[delete-object-instance-failure-mom-interaction]"
    "[receive-order-delete-object-instance-failure-mom-interaction]"
    "[rti.service.delete-failure-matrix]"
    "[rti.service.set-service-reporting-switch][rti.service.subscribe-interaction-class]"
  ...
  REQUIRE_NOTHROW(publisher->joinFederationExecution(
      L"delete-mom-failure-publisher", L"publisher", federationName));
  REQUIRE_NOTHROW(observer->connect(observerCallbacks, HLA_IMMEDIATE));
  REQUIRE_NOTHROW(observer->joinFederationExecution(
      L"delete-mom-failure-observer", L"observer", federationName));
  ...
  auto const verifyReport = [&](std::size_t index,
    ...
    REQUIRE(dynamic_cast<rti1516_2025::HLAunicodeString const&>(returned.get(2U)).get() == L"null");
  ...
  verifyReport(1U, objectValue, true, L"");
