...
std::wstring nextFederationName() {
  ...
  return L"timed-live-tso-regional-attribute-multi-recipient-negotiated-confirmation-cancel-" +
...
TEST_CASE(
    "Embedded timed multi-recipient regional timestamped attribute update cancels retained negotiated owner confirmation after restore",
    ...
    "[rti.service.negotiated-attribute-ownership-divestiture][rti.service.cancel-negotiated-attribute-ownership-divestiture][rti.service.confirm-divestiture]"
  ...
  std::wstring const saveLabel =
      L"restore-live-explicit-source-regional-negotiated-confirmation-cancel";
  ...
  // Create two If Available acquisition candidates after restore.  The
  // publisher's negotiated search selects the first candidate, then must
  // continue with the retained clock candidate when that requester resigns.
  ...
  // Cancel the retained negotiated transfer after Request Divestiture
  // Confirmation has reached the owner, before Confirm Divestiture.
  ...
  auto const& unavailableReport =
      clockReports.ownershipAcquisitionReports.front();
  REQUIRE(unavailableReport.kind ==
          ReportingFederateAmbassador::AttributeOwnershipAcquisitionReport::Kind::unavailable);
  REQUIRE(unavailableReport.objectInstance == objectInstance);
  REQUIRE(unavailableReport.attributes == clockFlavorOnly);
  REQUIRE(variableLengthDataBytes(unavailableReport.tag) ==
          std::vector<unsigned char>(
              secondAcquisitionTagBytes,
              secondAcquisitionTagBytes + sizeof(secondAcquisitionTagBytes)));
