...
std::wstring nextFederationName() {
  ...
  return L"embedded-known-class-disabled-" +
...
TEST_CASE(
    ...
    "[callbacks][attribute-relevance-advisory][known-class-disabled][2025]"
  ...
  REQUIRE_NOTHROW(owner->joinFederationExecution(
      L"known-class-disabled-owner", L"owner", federationName));
  REQUIRE_NOTHROW(subscriber->joinFederationExecution(
      L"known-class-disabled-subscriber", L"subscriber", federationName));
  REQUIRE_FALSE(owner->getAdvisoriesUseKnownClassSwitch());
  ...
  // The receiver knows only Employee, but Advisories Use Known Class is
  // disabled.  The later Server-only declaration therefore remains visible
  // to the owner-directed advisory calculation through the registered class.
  REQUIRE_NOTHROW(subscriber->subscribeObjectClassAttributes(
      server,
      serverEfficiency,
      true));
  ...
  REQUIRE(ownerReports.turnUpdatesOnReports.size() == 1U);
  REQUIRE(ownerReports.turnUpdatesOnReports.front().objectInstance == objectInstance);
  REQUIRE(ownerReports.turnUpdatesOnReports.front().attributes == serverEfficiency);
  ...
  REQUIRE(ownerReports.turnUpdatesOffReports.front().objectInstance == objectInstance);
  REQUIRE(ownerReports.turnUpdatesOffReports.front().attributes == serverEfficiency);

  REQUIRE_NOTHROW(subscriber->unsubscribeObjectClassAttributes(employee, employeeName));
  while (owner->evokeMultipleCallbacks(0.0, 0.0)) {
  }
  REQUIRE(ownerReports.turnUpdatesOffReports.size() == 2U);
  REQUIRE(ownerReports.turnUpdatesOffReports.back().objectInstance == objectInstance);
  REQUIRE(ownerReports.turnUpdatesOffReports.back().attributes == employeeName);
