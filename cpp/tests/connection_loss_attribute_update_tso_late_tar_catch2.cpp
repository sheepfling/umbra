...
#error "The late-TAR connection-loss tests require the Umbra source directory."
#endif
...
TEST_CASE(
    "Embedded transport loss protects a late cutoff attribute reflection from automatic object cleanup",
    ...
    "[connection-lost-tso-cutoff][late-tar]"
  ...
  auto const federationName = std::wstring{L"connection-loss-attribute-tso-late-tar"};
  auto const fomModule = (std::filesystem::path(UMBRA_SOURCE_DIRECTORY) /
                          ...
                          "examples" / "RestaurantFOMmodule-2025.xml")
                             .wstring();
  unsigned char const valueBytes[] = {0x4C, 0x41, 0x54, 0x45};
  unsigned char const tagBytes[] = {0x4C, 0x41, 0x54, 0x45, 0x2D, 0x54, 0x53, 0x4F};
  ...
  REQUIRE_NOTHROW(lostFederate = lost->joinFederationExecution(
      L"late-cutoff-lost-attribute-publisher", L"publisher", federationName));
  REQUIRE_NOTHROW(surviving->joinFederationExecution(
      L"late-cutoff-surviving-attribute-subscriber", L"subscriber", federationName));
  ...
  // The loss occurs before the constrained survivor asks for time 6.  Its
  // late TAR must still release the accepted TSO reflection before the grant,
  // while the DELETE_OBJECTS cleanup remains behind the next receive-order TAR.
  ...
  REQUIRE_NOTHROW(umbra::detail::failEmbeddedTransportConnectionForTesting(
      *lost,
      L"late timestamped attribute cutoff transport fault"));
  REQUIRE(survivingReports.attributeReflectionReports.empty());
  ...
  auto const& reflection = survivingReports.attributeReflectionReports.front();
