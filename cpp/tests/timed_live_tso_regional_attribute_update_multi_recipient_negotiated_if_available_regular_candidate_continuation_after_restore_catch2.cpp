...
std::wstring nextFederationName() {
  ...
  return L"timed-live-tso-regional-attribute-multi-recipient-negotiated-if-available-regular-candidate-" +
...
TEST_CASE(
    "Embedded timed multi-recipient regional timestamped attribute update continues from an If Available request to a regular candidate after restore",
    ...
    "[tso-regional-attribute-update-timed-negotiated-if-available-regular-candidate-state]"
    ...
    "[rti.service.attribute-ownership-acquisition][rti.service.attribute-ownership-acquisition-if-available]"
  ...
  // Create two mixed acquisition candidates after restore.  The publisher's
  // negotiated search selects the first If Available candidate, then must
  // continue with the retained regular clock candidate when that requester
  // resigns.
  ...
  // Reissuing the negotiated request must select the retained regular clock
  // candidate and carry its acquisition tag to the owner's confirmation.
  REQUIRE_NOTHROW(publisher->negotiatedAttributeOwnershipDivestiture(
      objectInstance,
      flavorOnly,
      negotiatedTag));
