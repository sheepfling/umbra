...
#error "The joined-federate MOM deletable-object-count tests require the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"federation-joined-mom-deletable-count-" +
...
TEST_CASE(
    "Embedded joined-federate MOM exposes deletable object count",
    "[integration][development-profile][federation-management][mom]"
    "[periodic-mom][object-management][ownership-management]"
    "[request-attribute-value-update][send-interaction]"
  ...
  auto const observerFederate = observer->joinFederationExecution(
      L"mom-deletable-count-observer", L"observer", federationName);
  ...
  auto const deletableCountAttribute = observer->getAttributeHandle(
      momClass, standard_hla::mom::object_instances_that_can_be_deleted);
  ...
  REQUIRE(deletableCountAttribute.isValid());
  REQUIRE(reliable.isValid());
  REQUIRE_NOTHROW(observer->subscribeObjectClassAttributes(
      momClass,
      AttributeHandleSet{federateHandleAttribute, deletableCountAttribute},
      true));

  auto const ownerFederate = owner->joinFederationExecution(
      L"mom-deletable-count-owner", L"owner", federationName);
  ...
  auto requestDeletableCount = [&] {
    ...
    auto const reflectionIterator = std::find_if(
        ...
        [&](ReportingFederateAmbassador::AttributeReflectionReport const& report) {
          return report.objectInstance == ownerMomObject &&
              report.attributeValues.contains(deletableCountAttribute);
        });
    ...
    REQUIRE_NOTHROW(decoded.decode(
        reflection.attributeValues.at(deletableCountAttribute)));
    return decoded.get();
  };

  REQUIRE(requestDeletableCount() == 0);
  ...
  REQUIRE(requestDeletableCount() == 1);
  ...
  REQUIRE_NOTHROW(periodicCount.decode(
      periodicReflection->attributeValues.at(deletableCountAttribute)));
