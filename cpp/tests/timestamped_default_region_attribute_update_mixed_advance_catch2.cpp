...
#error "The mixed default-region timestamped attribute-update test requires the Umbra source directory."
#endif
...
std::wstring nextFederationName() {
  ...
  return L"timestamped-default-region-attribute-update-mixed-advance-" +
...
TEST_CASE(
    "Embedded mixed default-region timestamped attribute updates deliver before FQR TARA and NMRA grants",
    "[integration][development-profile][object-management][ddm][time-management]"
    "[timestamped-default-region-attribute-update][default-region][tso][flush-queue-request]"
  ...
  unsigned char const valueBytes[] = {0x44, 0x45, 0x46, 0x2D, 0x41};
  unsigned char const tagBytes[] = {0x44, 0x45, 0x46, 0x2D, 0x46};
  ...
  REQUIRE_NOTHROW(publisherHandle = publisher->joinFederationExecution(
      L"mixed-default-region-attribute-publisher", L"publisher", federationName));
  REQUIRE_NOTHROW(fqr->joinFederationExecution(
      L"mixed-default-region-attribute-fqr", L"subscriber", federationName));
  REQUIRE_NOTHROW(tara->joinFederationExecution(
      L"mixed-default-region-attribute-tara", L"subscriber", federationName));
  REQUIRE_NOTHROW(nmra->joinFederationExecution(
      L"mixed-default-region-attribute-nmra", L"subscriber", federationName));
  ...
  // subscriber sees the private default source region and receives an
  // explicitly supplied, empty sent-region set in its reflection metadata.
  ObjectInstanceHandle objectInstance;
  ...
  auto requireDefaultRegionReflection = [&](auto const& report) {
  ...
  requireDefaultRegionReflection(fqrReports.attributeReflectionReports.front());
  requireDefaultRegionReflection(taraReports.attributeReflectionReports.front());
  requireDefaultRegionReflection(nmraReports.attributeReflectionReports.front());
